...
{ 
  LockSched();
	...
	if (ind == _CurIndex-_CurBase) 
	{
		...
		{
			if (file->ws_size) 
				DrawString(file->ws_size,TXT_X+5,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,NormalFont,TEXT_ALIGNLEFT,
					(char*)&Colors[clInfoTxt],0);

                        if(file->ws_time && file->ftype != TYPE_ZIP_FILE)
                        {
                            // рисовать будем посередине + отступ на 1/2 символа
//                            int posX = (ITM_X2 - ITM_B - 2 - TXT_X) / 2 + TXT_X + txt_h / 2;
                            DrawString(file->ws_time, TXT_X+5+Get_WS_width(file->ws_size,NormalFont)+15,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,NormalFont,TEXT_ALIGNLEFT,
                                       		(char*)&Colors[clInfoTxt],0);
                        }
                        ...
                        if (file->ftype == TYPE_ZIP_FILE && file->ws_ratio)
			{
				// рисовать будем посередине + отступ на 1/2 символа
				int posX = (ITM_X2 - ITM_B - 2 - TXT_X) / 2 + TXT_X + txt_h / 2;
				DrawString(file->ws_ratio, posX+5 ,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,NormalFont,TEXT_ALIGNLEFT,
					(char*)&Colors[clInfoTxt],0);
			}
		}
                else
                  if(file->ws_time && file->ftype != TYPE_ZIP_FILE)
                    {
                       DrawString(file->ws_time, TXT_X+5,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,NormalFont,TEXT_ALIGNLEFT,
                                       		(char*)&Colors[clInfoTxt],0);
                    }
		if (file->ws_attr)
	...
	if (fn)
        {
                if (file->attr & FA_DIRECTORY) 
                  ...
                  wsprintf(fnbuf,"%w",fn);
		DrawString(fnbuf,TXT_X+5,y+ITM_B+1,ITM_X2-ITM_B-2,y+ITM_B+txt_h,BoldFont,TEXT_ALIGNLEFT,
			(char*)&Colors[tc],0);
        }
  UnlockSched();
}
...
{
	if (!guibuf) return;
	DrawRoundedFrame(0,YDISP1,scr_w-1,scr_h-1,0,0,0,(char*)&Colors[clBD],(char*)&Colors[clBG]);
