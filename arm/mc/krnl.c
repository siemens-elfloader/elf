...
void S_ZipOpenFile(char* ext, int isAltOpen)
{
	...
	if (res == UNZ_OK)
	{
          if (isAltOpen)
          {
                 ...
                 str_2ws(wsext, ext, 255);
                 int uid = GetExtUid_ws(wsext);
                 if (!uid)
                 {
                   str_2ws(wsext, txt_ext, 255);
	...
	if (file && pathbuf && strlen(file->sname))
	{
		...
		else
		{
			...
			if (wsbuf)
			{
				...
				{
                                  if ( (isAltOpen) && (isItZip != 1) )
                                  {
                                        WSHDR* wsext = AllocWS(256);
                                        str_2ws(wsext, sz, 255);
                                        int uid = GetExtUid_ws(wsext);
                                        if (!uid)
                                        {
                                          str_2ws(wsext, txt_ext, 255);
                                        ...
                                        if (!regext->altproc)
                                        {
                                          str_2ws(wsext, txt_ext, 255);
                                          uid = GetExtUid_ws(wsext);
                                          regext = get_regextpnt_by_uid(uid);
                                        }
...
void DoNatExpl()
{
 ...
 // myExplorerStruct->fixed_directory_key = 0x26;
  ...
  myExplorerStruct->full_filename = ws;
  myExplorerStruct->path_to_file = ws1;
  myExplorerStruct->file_name = ws2;
  ...
  if (_CurIndex >= 0)
  {
	...
	{
          CurFullPath(file->sname);
          wsprintf(ws,percent_t,pathbuf);
          wsprintf(ws1,percent_t,_CurPath);
          wsprintf(ws2,percent_t,file->sname);
