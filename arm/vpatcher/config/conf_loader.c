...
char cpath[128] = "0:\\ZBin\\etc\\VPatcher.bcfg";

void InitConfig()
{
  if (LoadConfigData(cpath)<0)
  {
    cpath[0] = '4';
    LoadConfigData(cpath);
  }
}
...
void OpenConfig()
{
     ...
     wsprintf(path_ws, cpath);
     ExecuteFile(path_ws, 0, "");
     FreeWS(path_ws);
}
