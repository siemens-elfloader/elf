...
{
  PP *pp=(PP *)png_get_io_ptr(png_ptr);
...
IMGHDR *read_pngimg(const char *buf)
{
  ...
  pp.img_h=img_hc=(IMGHDR *)malloc(sizeof(IMGHDR));
  
  {
    int *iimg=(int *)(pp.img=(char *)malloc(width*height*sizeof(int)));
...
{
  int ensize;
  IMGHDR * img_hc=(IMGHDR *)malloc(sizeof(IMGHDR));
...
IMGHDR *read_jpgimg(const char *buf, int mod)
{
  ...
  img_hc=(IMGHDR *)malloc(sizeof(IMGHDR));
  iimg=img=(unsigned int *)malloc(img_w*img_h*sizeof(int));
...
{
  IMGHDR * img_hc=(IMGHDR *)malloc(sizeof(IMGHDR));
  unsigned short *iimg=(unsigned short *)malloc(width*height*sizeof(unsigned short));
  img_hc->w=width;
  img_hc->h=height;
  img_hc->bpnum=8;
  img_hc->bitmap=(char *)iimg;
