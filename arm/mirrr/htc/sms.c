...
// 	WSHDR *tws=AllocWS(wstrlen(ws0)*2);
	char *txt=(char *)malloc(wstrlen(ws0)*2);
...
// 	ShowMSG(1,(int)txt);
	WSHDR *tws=(WSHDR *)AllocWS(strlen(txt));
	wsprintf(tws,txt);
	mfree(txt);
	return tws;
}
...
{
// 	int cnt, len;
	SMS_LEN *tt=(SMS_LEN *)malloc(sizeof(SMS_LEN));
	if(is7bit(t))
	{
		tt->is7=1;
		tt->len=wstrlen(t);
...
int read_smsdat()
{
	...
	unsigned int er;
	char *smsdat;
	if ((f=fopen("0:\\system\\sms\\sms.dat",A_ReadOnly+A_BIN,0,&er))!=-1)
	{
		int fsz=lseek(f,0,S_END,&er,&er)-2;
		scnt.all=fsz/sizeof(PDU);
		SMSD=(SMS_DATA *)malloc(sizeof(SMS_DATA)*scnt.all);
		lseek(f, 2, 0, &er, &er);
		smsdat=(char *)malloc(sizeof(PDU)*scnt.all);
...
// 		if(SMSD[i].n==1)
	...
	if(scnt.in) _in=(char *)malloc(scnt.in*5);
	if(scnt.out) _out=(char *)malloc(scnt.out*5);
	if(scnt.d) _draft=(char *)malloc(scnt.d*5);
