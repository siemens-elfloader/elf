 ...
 *
 *   typedef void *_Rmtx;       // Lock info object
 *   void _Mtxinit(_Rmtx *);    // Initialize a lock
 *   void _Mtxdst(_Rmtx *);     // Destroy a lock
 *   void _Mtxlock(_Rmtx *);    // Lock a lock
 *   void _Mtxunlock(_Rmtx *);  // Unlock a lock
 * 
 ...
 *   _DLIB_THREAD_LOCK_ONCE_TYPE_INIT
 *
 * and, if an external TLS interface is used, the following functions must
 * be defined:
 *   int _Tlsalloc(_Tlskey_t*, _Tlsdtor_t); // Allocate a TLS element
 *   int _Tlsfree(_Tlskey_t);               // Free a TLS element
 *   int _Tlsset(_Tlskey_t, void *);        // Set a TLS element
 *   void *_Tlsget(_Tlskey_t);              // Get a TLS element
 *
 */
...
#define _DLIB_TLS_QUAL 
  ...
  typedef int _Tlskey_t;
    ...
    __INTRINSIC int _Tlsalloc(_Tlskey_t*, _Tlsdtor_t);
    __INTRINSIC int _Tlsfree(_Tlskey_t);
    __INTRINSIC int _Tlsset(_Tlskey_t, void *);
    __INTRINSIC void *_Tlsget(_Tlskey_t);
   ...
   * functions:
   *   extern void *_Allocate_TLS(void);
   *   extern void  _Deallocate_TLS(void *p);
   ...
   *   __TLSp = _Allocate_TLS();
   *
   * at thread_destruction:
   *   #include <DLib_Threads.h>
   *   _Deallocate_TLS(__TLSp);
   *
