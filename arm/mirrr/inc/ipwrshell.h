...
typedef TFIREWALLTYPES TipwRshellFirewallTypes;




typedef void __fastcall (__closure* TipwRshellConnectedEvent)(System::TObject* Sender,
                                    int StatusCode,
                                    AnsiString& Description);
typedef void __fastcall (__closure* TipwRshellDisconnectedEvent)(System::TObject* Sender,
                                    int StatusCode,
                                    AnsiString& Description);
typedef void __fastcall (__closure* TipwRshellErrorEvent)(System::TObject* Sender,
                                    int ErrorCode,
                                    AnsiString& Description);
typedef void __fastcall (__closure* TipwRshellStderrEvent)(System::TObject* Sender,
                                    char * Text, USHORT lenText,
                                    bool EOL);
typedef void __fastcall (__closure* TipwRshellStdoutEvent)(System::TObject* Sender,
                                    char * Text, USHORT lenText,
                                    bool EOL);


class PACKAGE TipwRshell : public IPWCore
{
private:
  LONG reserved;
  TipwRshellConnectedEvent FOnConnected;
  TipwRshellDisconnectedEvent FOnDisconnected;
  TipwRshellErrorEvent FOnError;
  TipwRshellStderrEvent FOnStderr;
  TipwRshellStdoutEvent FOnStdout;
...
public:
  ...
  TipwRshellFirewallTypes __fastcall FFirewallType();
  void __fastcall FSetFirewallType(TipwRshellFirewallTypes iFirewallType);
  ...
  AnsiString __fastcall FLocalUser();
  void __fastcall FSetLocalUser(AnsiString lpszLocalUser);
  ...
  virtual __fastcall TipwRshell(TComponent* Owner);
  virtual __fastcall TipwRshell(TComponent* Owner, bool Registered, LPSTR lpKey=IPWORKS_OEMKEY_21);
  virtual __fastcall ~TipwRshell();
...
__published:
  ...
  //PROPERTIES
    ...
    __property TipwRshellFirewallTypes FirewallType = {
                    read=FFirewallType,
    ...
    __property AnsiString LocalUser = {
                    read=FLocalUser,
                    write=FSetLocalUser,
  ...
  //EVENTS
    //Fired immediately after a connection completes (or fails).
    __property TipwRshellConnectedEvent OnConnected = {read=FOnConnected, write=FOnConnected};
    //Fired when a connection is closed.
    __property TipwRshellDisconnectedEvent OnDisconnected = {read=FOnDisconnected, write=FOnDisconnected};
    //Information about errors during data delivery.
    __property TipwRshellErrorEvent OnError = {read=FOnError, write=FOnError};
    //Fired when data (complete lines) come in through stderr.
    __property TipwRshellStderrEvent OnStderr = {read=FOnStderr, write=FOnStderr};
    //Fired when data (complete lines) come in through stdout.
    __property TipwRshellStdoutEvent OnStdout = {read=FOnStdout, write=FOnStdout};


}; //end class TipwRshell
