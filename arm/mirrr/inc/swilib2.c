...
#define _SWILIB_C
//#define ELKA
#include "..\Rebel Editor\readimg.h"
...
int plott(char liter, int font, int x, int y, int fcolor); //рисуем символ шрифта (название бралось от пОнта, к черчению не имеет никакого отношения)
...
if (((y+j-yRect)<176) && ((x+i-xRect)<132) && ((y+j-yRect)>=0) && ((x+i-yRect)>=0) &&(i>=xRect)&&(j>=yRect)&&(i<xRect+xxRect)&&(j<yRect+yyRect))
...
if (((y+j-yRect)<176) && ((x+i-xRect)<132) && ((y+j-yRect)>=0) && ((x+i-xRect)>=0) 
    &&(i>=xRect)&&(j>=yRect)&&(i<xRect+xxRect)&&(j<yRect+yyRect))
 {
   
 scr[(y+j-yRect)*132+i+x-xRect]=RGB16(pic[((j)*onse->w+i)*4+0],pic[((j)*onse->w+i)*4+1],pic[((j)*onse->w+i)*4+2])  ;
...
int light2img(IMGHDR * timage, int povtor, int shir){ //20.29   //25 - 140
if (!timage) return 1;
...
IMGHDR *morimg(IMGHDR * onse, int rf)
{
  if (!onse) return 0;
IMGHDR *outkasta;
outkasta = CreateImgr((onse->w+rf), (onse->h+rf));
 if (EFFECT==1) drawimg2img(onse, outkasta, 0, 0, 0, 0,onse->w, onse->h);  
...
if (((y+j-yRect)<176) && ((x+i-xRect)<132) && ((y+j-yRect)>=0) && ((x+i-xRect)>=0) 
 ...
 if (pic[(j)*onse->w+i]!=0)  scr[(y+j-yRect)*132+i+x-xRect]=pic[(j)*onse->w+i];
 else scr[(y+j-yRect)*132+i+x-xRect]=sColor;
 }}
 return 0;
}*/
...
int x_str(char *text, int font){
  int i=0, cas=0;
  while(text[i]){
cas+=x_char (text[i], font);
   ++i;
  }
  return  cas;
}
...
void hexer(const char *bufw){

    char *out; 
    out = malloc((lenta*4)+21);
    sprintf(out,"char *resstr = \x22");
    for (int i=0; i<lenta; i++){
    //
    sprintf(out,"%s\\x%s",out, hex(bufw[i]));
    }
