...
_END_C_LIB_DECL
  ...
  #define _FPCOMP(x, y) _CARG2I(x, y, _Dcomp, _F_FNAME(Dcomp), _L_FNAME(Dcomp))
  #define fpclassify(x) _CARGI(x, _Dtest, _F_FNAME(Dtest), _L_FNAME(Dtest))
  #define signbit(x)    _CARGI(x, _Dsign, _F_FNAME(Dsign), _L_FNAME(Dsign))
  ...
  __INTRINSIC int _Dcomp(double, double);
  __INTRINSIC int _FDcomp(float, float);
  __INTRINSIC int _LDcomp(long double, long double);
  #pragma inline
  int _Dcomp(double x, double y)
  {
    if (isnan(x) || isnan(y))
    {
      return 0;
    ...
    #pragma inline
    int _LDcomp(long double x, long double y)
    {
      if (isnan(x) || isnan(y))
      {
        return 0;
