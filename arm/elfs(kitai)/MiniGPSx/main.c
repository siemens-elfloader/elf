...
void Savenew()
{
  ...
  WSHDR *ws0 = AllocWS(192);
  wsprintf(ws0,"%t%u-%u",drun,Res.ci,Res.lac);
  unsigned int len = ws0->wsbody[0];
...
void load_name()
{
  ...
  else
  {
    ...
    if(errcode)
    {
      wsprintf(corr_name,"%t %u","´íÎó", errcode);
    }
    else
    {
      //str_len=wstrlen(tmostr)-2;
      //wstrcpybypos(corr_name,tmostr,0,str_len);
	  wstrcpy(corr_name,tmostr);
    }
    FreeWS(tmostr);
    fclose(hFile, &errcode);
  }
