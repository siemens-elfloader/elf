...
#define READ(_gif,_buf,_len)                                     \
  (((GifFilePrivateType*)_gif->Private)->Read ?                   \
    ...
    fread(((GifFilePrivateType*)_gif->Private)->File,_buf,_len,0))
...
GifFileType *DGifOpenFileName(const char *FileName)
{
    ...
    FileHandle = fopenA(FileName,A_ReadOnly+A_BIN,P_READ+P_WRITE,0);
    if(FileHandle==-1)
    {
    ...
    if(GifFile == (GifFileType *)NULL) fclose(FileHandle,0);
    return GifFile;
}
