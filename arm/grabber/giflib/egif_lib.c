...
#define WRITE(_gif,_buf,_len)   \
  (((GifFilePrivateType*)_gif->Private)->Write ?    \
   ...
   fwrite(((GifFilePrivateType*)_gif->Private)->File,_buf, _len,0 ))
...
GifFileType *EGifOpenFileName(char *FileName, int TestExistance)
{
    ...
    if (TestExistance) FileHandle = fopenA(FileName,A_WriteOnly+A_BIN+A_Create,P_READ+P_WRITE,0);//?????
...
int EGifCloseFile(GifFileType *GifFile)
{
    ...
    if (Private) {
        if (Private->HashTable) mfree((char *) Private->HashTable);
	    mfree((char *) Private);
    }
    mfree(GifFile);
