...
inline int SaveConfigData(const char *fname)
{
  int f;
  unsigned int ul;
  int result=-1;
...
#pragma segment="CONFIG_C"
inline int LoadConfigData(const char *fname)
{
  int f;
  unsigned int ul;
  char *buf;
...
void InitConfig()
{
  if (LoadConfigData("4:\\ZBin\\etc\\E X-O_Online.bcfg")<0)
  {
    LoadConfigData("0:\\ZBin\\etc\\E X-O_Online.bcfg");
  }
}
