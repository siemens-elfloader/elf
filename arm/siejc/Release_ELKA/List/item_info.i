...
HEADER_DESC info_hdr={0,0,0,0,0,(int)"Контакт инфо",0x7FFFFFFF};
...
void Disp_Info(TRESOURCE* ResEx)
{
  ...
  wsprintf(ws_info,"");
  ConstructEditControl(&ec,0,0,ws_info,256);
  ...
  if(ResEx->entry_type==T_NORMAL)   ascii2ws(ws_info, "Элемент ростера");
  if(ResEx->entry_type==T_VIRTUAL)  ascii2ws(ws_info, "Оффлайн или контакт из транспорта");
  if(ResEx->entry_type==T_CONF_ROOT)ascii2ws(ws_info, "Конференция");
  if(ResEx->entry_type==T_CONF_NODE)ascii2ws(ws_info, "Участник конференции");
  if(ResEx->entry_type==T_TRANSPORT)ascii2ws(ws_info, "Данный контакт является транспортом Jabber");
  if(ResEx->entry_type==T_GROUP)    ascii2ws(ws_info, "Группа ростера");
  ...
  if(ResEx->entry_type==T_CONF_ROOT)
  {
    ...
    if(TmpMUC)
    {
      if(TmpMUC->muctema)
      {
        ascii2ws(ws_info, "Тема:");
        ConstructEditControl(&ec,1,0x40,ws_info,256);
        AddEditControlToEditQend(eq,&ec,ma);
        utf8_2ws(ws_info, TmpMUC->muctema, strlen(TmpMUC->muctema)*2);
        ConstructEditControl(&ec,3,0x40,ws_info,wstrlen(ws_info));
        AddEditControlToEditQend(eq,&ec,ma);
      }
    }
  }
...
char *Lookup_Known_Vars(char *var_name)
{
  if(!Known_Features)
  {
    ...
    volatile int hF=fopen(path ,2 + 0x8000,0x80 + 0x100, &io_error);
...
void Disp_From_Disco(char *jid, XMLNode *info)
{
  ...
  ascii2ws(ws_info, "Возможности клиента:");
  ConstructEditControl(&ec,1,0x40,ws_info,256);
  AddEditControlToEditQend(eq,&ec,ma);
