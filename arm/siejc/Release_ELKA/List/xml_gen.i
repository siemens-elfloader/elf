...
XML_CreateNode(char *name, char *value)
{
	...
	(memset((node), '\0', (sizeof(XMLNode))), (void) 0);
...
XML_Set_Attr_Value(XMLNode *node, char *attr_name, char *attr_value)
{
	...
	if (tail) {
		...
		(memset((p), '\0', (sizeof(XMLAttr))), (void) 0);
...
XML_Get_Node_As_Text(XMLNode *node)
{
	...
	while (attr && attr->name && attr->param) {
		buf[occ_len++]=' ';
		l = strlen(attr->name);
		buf = chk_realloc_buf(buf, &buf_len, buf_len - occ_len, l);;
	...
	if (node->subnode) {
		buf = chk_realloc_buf(buf, &buf_len, buf_len - occ_len, 1);;
		buf[occ_len++] = '>';
		...
		l = strlen(node->name);
		buf = chk_realloc_buf(buf, &buf_len, buf_len - occ_len, l+3);;
		buf[occ_len++] = '<';
		buf[occ_len++] = '/';
		strncpy(buf + occ_len, node->name, buf_len - occ_len);
		occ_len+=l;
		...
		if (node->value) {
			...
			buf = chk_realloc_buf(buf, &buf_len, buf_len - occ_len, 2);;
			buf[occ_len++] = '/';
			buf[occ_len] = '>';
		}
	}
