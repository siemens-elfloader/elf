...
extern RGBA MESSAGEWIN_GCHAT_BGCOLOR_1; 
extern RGBA MESSAGEWIN_GCHAT_BGCOLOR_2; 
...
extern RGBA CONTACT_BG_0; 
extern RGBA CONTACT_BG_1; 
extern RGBA lineColor; 
extern RGBA borderColor; 

extern RGBA PRES_COLORS[12] ;
...
const char CMP_DATE[] = "Feb 23 2011";
const unsigned long PING_INTERVAL = 3*60*216; 
const char OS[] = "NewSGOLD_ELKA";

const char OS_postfix[] = "_ELF-Platform";


const char ipc_my_name[32]= "SieJC";
...
void _start_vibra()
{
  ...
  if(Is_Vibra_Enabled)
  {
    ...
    GBS_StartTimerProc(&tmr_vibra,216>>1,_stop_vibra);
  }
}
...
void _stop_vibra(void)
{
  ...
  if (--Vibra_Count)
  {
    GBS_StartTimerProc(&tmr_vibra,216>>1,_start_vibra);
  }
}
...
void create_connect(void)
{
  ...
  if(!IS_IP)
  {
    ...
    SMART_REDRAW();
    *socklasterr()=0;
    int err=async_gethostbyname(JABBER_HOST,&p_res,&DNR_ID); 
    if (err)
    {
...
unsigned int in_bytes_count = 0; 
unsigned int in_virt_bytes_count = 0; 
z_stream d_stream;                
z_stream c_stream;                
unsigned int out_bytes_count = 0; 
unsigned int out_virt_bytes_count = 0; 
char *Rstream_p;                  
int Rstream_n;                    
...
void get_answer(void)
{
  ...
  Rstream_p[Rstream_n]=0; 
  in_virt_bytes_count += Rstream_n;

  i=0; 
  j=0; 
  p=Rstream_p;
  if ((strstr(p,"</stream:stream>"))&&(My_Presence == 6)) QuitCallbackProc(0); 
  while((p=strstr(p,"<?xml version='1.0'"))) {i--; p++;} 
  p=Rstream_p;
  while((p=strstr(p,"<stream:stream"))) {i--; p++;} 
  ...
  while((c=*p++))
  {
    ...
    L_END:
      ...
      if ((!i)&&(!j))
      {
        ...
        memcpy(p=Rstream_p,Rstream_p+bytecount,(Rstream_n-=bytecount)+1); 
        GBS_SendMessage(0x4209,0xDEAE,0,tmp_buffer,sock); 
      }
    }
  }
}
...
Compression_Init_Stream()
{
  ...
  err = inflateInit2_((&d_stream), (15), "1.2.3", sizeof(z_stream));
  if (err) {
    Compression_Report_Error("Zlib inflate init", err);
    Jabber_state = JS_NOT_CONNECTED;
  }
  
  err = deflateInit_((&c_stream), (9), "1.2.3", sizeof(z_stream));
  if (err) {
...
void onRedraw(MAIN_GUI *data)
{
  ...
  DrawRoundedFrame(0,(24),scr_w-1,scr_h-1,0,0,0,
                   0,
                   (char *)(&(bgr_color)));
  ...
  Roster_DrawIcon(1, (24)+1, (int)mypic);
  DrawString(data->ws1,Roster_getIconWidth(mypic)+2,(24)+3,scr_w-4,scr_h-4-16,font_width,0,(char *)(&(font_color)),0);
  ...
  if(Jabber_state!=JS_ONLINE)
  {
    ...
    DrawString(data->ws1,1,(24)+3+GetFontYSIZE(font_width)+2,scr_w-4,scr_h-4-16,font_width,0,(char *)(&(font_color)),0);
  }
...
void Enter_SiepatchDB()
{
  ...
  Enter_Conference(room, nick, 0, DEFAULT_MUC_MSGCOUNT);
}
  ...
  if (msg->gbsmsg->msg==0x193)
  {
    ...
    case 0x0B:
      ...
      if(connect_state==2 && Jabber_state==JS_ONLINE && CList_GetActiveContact()->entry_type != T_GROUP)
        Init_Message(CList_GetActiveContact(), 0);
      break;
	...
	if (IDLE_ICON)
     {
      ...
      if (IsGuiOnTop((((int *)icsm)[0x2C/4]))) RedrawMainIcon();
     }
    }
    if(Quit_Required)
    {
...
void Check_Settings_Cleverness()
{
  if(!USE_SASL && USE_ZLIB)
    ShowMSG(0,(int)"ZLib не работает без SASL!");
}
...
int ReadDefSettings(char *elfpath)
{
  ...
  if ((f=fopen(elfpath,0+0x8000,0x80,&err))!=-1)
  {
    int rlen = fread(f,&def_set,sizeof(DEF_SETTINGS),&err);
    int end=lseek(f,0,2,&err,&err);
    fclose(f,&err);
    ...
    if(def_set.status<6) OnlineInfo.status =def_set.status;
...
int WriteDefSettings(char *elfpath)
{
  ...
  if ((f=fopen(elfpath,1+0x8000+0x100+0x200,0x100,&err))==-1) return 0;
...
int main(char *exename, char *fname)
{
  ...
  if(!IsGoodPlatform())
  {
    ShowMSG(1,(int)"Платформа не соответствует!");
    return 0;
  }
  ...
  extern TTime intimes;           
  extern TDate indates;           
  ...
  {
    if(AUTOSTATUS_TIME < 1) autostatus_time = 15000; 
    else autostatus_time = 216*60*AUTOSTATUS_TIME;
