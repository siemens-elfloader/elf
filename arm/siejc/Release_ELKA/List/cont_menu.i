...
HEADER_DESC contact_menuhdr={0,0,0,0,0,(int)contmenu_header,0x7FFFFFFF};
...
{
  8,contact_menu_onkey,contact_menu_ghook,0,
  menusoftkeys,
  &clmenu_skt,
  0x11,
...
char MA_Menu_Contents[10-1];
const char ma_menu_header[]= "muc#admin";
int mamenusoftkeys[]={0,1,2};
HEADER_DESC ma_menuhdr={0,0,0,0,0,(int)ma_menu_header,0x7FFFFFFF};
...
MENU_DESC ma_menu=
{
  8,ma_menu_onkey,ma_menu_ghook,0,
  mamenusoftkeys,
  &ma_menu_skt,
  0x10, 
  ma_menu_iconhndl,
...
{
  for(int i=0;i<10;i++)MA_Menu_Contents[i]=0;
}
...
if (MYMUCRES->muc_privs.aff<AFFILIATION_ADMIN) 
  {
   MA_Menu_Contents[n_items++]=1; 
  if(Act_contact->muc_privs.role==ROLE_VISITOR) 
  {
       MA_Menu_Contents[n_items++]=4;    
  }
  else  
  {
       MA_Menu_Contents[n_items++]=3;
  }
  ...
  {
  if(Act_contact->muc_privs.role!=ROLE_MODERATOR) 
  {
       MA_Menu_Contents[n_items++]=1; 
  }
  ...
  else if(Act_contact->muc_privs.aff<AFFILIATION_ADMIN) 
  {
       MA_Menu_Contents[n_items++]=3;
  }
  ...
  {
       MA_Menu_Contents[n_items++]=6;
  }
  if(Act_contact->muc_privs.role!=ROLE_MODERATOR) 
  {
      MA_Menu_Contents[n_items++]=7;
  }
  else if(Act_contact->muc_privs.aff<AFFILIATION_ADMIN) 
  {
      MA_Menu_Contents[n_items++]=8;
  }
  if(Act_contact->muc_privs.aff!=AFFILIATION_ADMIN&&MYMUCRES->muc_privs.aff==AFFILIATION_OWNER) 
  {
      MA_Menu_Contents[n_items++]=9;
  }
  if(Act_contact->muc_privs.aff!=AFFILIATION_OWNER&&MYMUCRES->muc_privs.aff==AFFILIATION_OWNER) 
  {
      MA_Menu_Contents[n_items++]=10; 
  }
  }
  patch_rect(&ma_menuhdr.rc,0,(24),ScreenW()-1,HeaderH()+(24));
...
char CONTC_Menu_Contents[5-1];
const char contc_menu_header[]= "Авторизация";
int contcmenusoftkeys[]={0,1,2};
HEADER_DESC contc_menuhdr={0,0,0,0,0,(int)contc_menu_header,0x7FFFFFFF};
...
{
  8,contc_menu_onkey,contc_menu_ghook,0,
  contcmenusoftkeys,
  &contc_menu_skt,
  0x10, 
  contc_menu_iconhndl,
...
{
  for(int i=0;i<5;i++)CONTC_Menu_Contents[i]=0;
}
...
void Disp_Cont_Menu()
{
  ...
  InitContMenuArray();
  int n_items=0;
  CONTC_Menu_Contents[n_items++]=1;

   {
       CONTC_Menu_Contents[n_items++]=4; 
   }

   {
     CONTC_Menu_Contents[n_items++]=3;
     CONTC_Menu_Contents[n_items++]=2;
   }
  patch_rect(&contc_menuhdr.rc,0,(24),ScreenW()-1,HeaderH()+(24));
    ...
    case 1:
      {
        ...
        Send_Leave_Conference(room->JID, 0);
        break;
      }
    case 12:
      {
...
void InitMenuArray()
{
  for(int i=0;i<12;i++)Menu_Contents[i]=0;
}
...
void contact_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  case 12:
    {
      strcpy(test_str,"Очистить сообщения");
      break;
    }
  ...
  case 9:
    {
      strcpy(test_str,"Бездействие");
      break;
    }

  case 5:
    {
      strcpy(test_str,"История");
      break;
    }
  case 3:
    {
      if(Act_contact->status==6)
      {
...
void Init_Icon_array()
{
  ...
  for(int i=0;i<=12;i++)cmS_ICONS[i]=0;
  cmS_ICONS[1]=(int)ICON_CONF_LEAVE;
  cmS_ICONS[2]=(int)ICON_QUERY_VERSION;
  cmS_ICONS[4]=(int)ICON_QUERY_DISCO;
  cmS_ICONS[5]=(int)ICON_HISTORY_OPEN;
  cmS_ICONS[6]=(int)ICON_QUERY_TIME;
  cmS_ICONS[7]=(int)ICON_QUERY_VCARD;
  cmS_ICONS[9]=(int)ICON_QUERY_LASTACTIV;  
  cmS_ICONS[3]=(int)ICON_LOGIN_LOGOUT;
  cmS_ICONS[8]=(int)ICON_MUC_ADMIN;
  cmS_ICONS[10]=(int)  ICON_SUBSCRIBE_MENU;
  cmS_ICONS[11]=(int)  ICON_SUBSCRIBE_MENU;
  cmS_ICONS[12]=(int)ICON_SUBSCRIBE_MENU;
}
...
void Disp_Contact_Menu()
{
  ...
  {
    if(Act_contact->status!=6) Menu_Contents[n_items++]=1;
    Menu_Contents[n_items++]=5;
    Menu_Contents[n_items++]=4;
    if(Act_contact->total_msg_count) Menu_Contents[n_items++]=12;
  }

  if((Act_contact->entry_type==T_CONF_NODE)&&(Act_contact->status!=6))
  {
    TRESOURCE* MYMUCRES = CList_IsResourceInList(CList_FindMUCByJID(CList_FindContactByJID(Act_contact->full_name)->JID)->conf_jid);
    Menu_Contents[n_items++]=2;
    Menu_Contents[n_items++]=4;
    Menu_Contents[n_items++]=7;
    Menu_Contents[n_items++]=6;
    Menu_Contents[n_items++]=9;
  ...
  if((Act_contact->entry_type==T_NORMAL)||(Act_contact->entry_type==T_VIRTUAL))
  {
    ...
    Menu_Contents[n_items++]=6;
    Menu_Contents[n_items++]=9;
    Menu_Contents[n_items++]=10;
    Menu_Contents[n_items++]=11;
  }

  if(Act_contact->entry_type==T_TRANSPORT)
  {
    Menu_Contents[n_items++]=3;
    Menu_Contents[n_items++]=2;
    Menu_Contents[n_items++]=4;
    Menu_Contents[n_items++]=7;
    Menu_Contents[n_items++]=6;
  ...
  if(n_items)
  {
    patch_rect(&contact_menuhdr.rc,0,(24),ScreenW()-1,HeaderH()+(24));
...
int ma_menu_onkey(void *data, GUI_MSG *msg)
{
  ...
  if(msg->keys==0x18 || msg->keys==0x3D)
  {
    ...
    if(MA_Menu_Contents[i]==1)admin_cmd=ADM_KICK;
    if(MA_Menu_Contents[i]==2)admin_cmd=ADM_BAN;
    if(MA_Menu_Contents[i]==3)admin_cmd=ADM_VOICE_REMOVE;
    if(MA_Menu_Contents[i]==4)admin_cmd=ADM_VOICE_GRANT;
    if(MA_Menu_Contents[i]==5)admin_cmd=ADM_PARTICIPANT;
    if(MA_Menu_Contents[i]==6)admin_cmd=ADM_MEMBER;
    if(MA_Menu_Contents[i]==7)admin_cmd=ADM_MODERATOR;
    if(MA_Menu_Contents[i]==8)admin_cmd=ADM_MODERATOR_REMOVE;
    if(MA_Menu_Contents[i]==9)admin_cmd=ADM_ADMIN;
    if(MA_Menu_Contents[i]==10)admin_cmd=ADM_OWNER;
...
void ma_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  case 5:
    {
      strcpy(test_str,"Отнять членство");
      break;
    }

  case 6:
    {
      strcpy(test_str,"Дать членство");
      break;
    }
  case 8:
    {
      strcpy(test_str,"Отнять модератора");
      break;   
    }
    
  case 7:
    {
      strcpy(test_str,"Дать модератора");
      break;
    }

  case 9:
    {
      strcpy(test_str,"Дать админа");
      break;
    }

  case 10:
    {
      strcpy(test_str,"Дать владельца");
      break;
    }
  }
...
int reason_onkey(GUI *gui, GUI_MSG *msg)
{
  ...
  {
    if ((msg->gbsmsg->submess == 0x0B) || (msg->keys == 0x0018))
    {
      EDITCONTROL ec;
      char *s = malloc(256);
      int len;
...
void reason_ghook(GUI *gui, int cmd)
{
  if (cmd == 7)
  {
    static SOFTKEY_DESC sk = {0x0018, 0x0000, (int)"OK"};
    SetSoftKey(gui, &sk, 0);
  }
}
...
HEADER_DESC reason_hdr={0,0,0,0,0,(int)"Причина",0x7FFFFFFF};
