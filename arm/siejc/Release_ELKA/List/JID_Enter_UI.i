...
void ed_grouplist_handler(USR_MENU_ITEM *item)
{
  ...
  if (item->type==0)
  {
      if (i<gr_str->count) utf8_2ws(item->ws,GetGroupNameByID(i+1),80);
  }
  if (item->type==1)
  {
      if (i<gr_str->count)
      {
	char *p=GetGroupNameByID(i+1); 
...
if (msg->gbsmsg->msg==0x193)
  {
    if (msg->gbsmsg->submess==0x1A)
    {
      if (EDIT_GetFocus(data)==8)
      {
    jid_set.jid_ask=!jid_set.jid_ask;  
    jid_set.jid_del=!jid_set.jid_del;  
    CutWSTR(jews,0);
    wsAppendChar(jews, jid_set.jid_del?0xE116:0xE117);
...
void jed1_ghook(GUI *data, int cmd)
{
 ...
 if(jid_jid)
 {
  ...
  SendIq(0, "set", di, "jabber:iq:roster", xml_item);
  if((!jid_set.jid_add)&&(jid_set.jid_ask))
    {
      Send_ShortPresence(jid_jid,8);
    }
 } else ShowMSG(1,(int)"Нет JIDа!");
...
HEADER_DESC jed1_hdr={0,0,131,21,0,(int)"Контакт:",0x7FFFFFFF};
...
void Disp_JID_Enter_Dialog(CLIST* ClEx)
{
  ...
  jid = ClEx->JID;
  name = ClEx->name;
  if(ClEx->group!=0) group = GetGroupNameByID(ClEx->group); 
  }
  else
  {
  jid_set.jid_add=0;    
  ...
  ascii2ws(jews, "JID:");
  ConstructEditControl(&ec,1,0x40,jews,256);
  AddEditControlToEditQend(eq,&ec,ma);

  PrepareEditControl(&ec);
  utf8_2ws(jews, jid, 64);
  if(jid_set.jid_add)ConstructEditControl(&ec,1,0x40,jews,128);
   else ConstructEditControl(&ec,3,0x40,jews,128);
  AddEditControlToEditQend(eq,&ec,ma);  
  ...
  ConstructEditControl(&ec,3,0x40,jews,80);     
  AddEditControlToEditQend(eq,&ec,ma);  
  
  PrepareEditControl(&ec);
  ascii2ws(jews, "Группа:");
  ConstructEditControl(&ec,1,0x40,jews,256);      
  AddEditControlToEditQend(eq,&ec,ma);  
  ...
  if (!jid_set.jid_add)
  {
    ascii2ws(jews, "Запросить подписку:");
    ConstructEditControl(&ec,1,0x40,jews,256);      
    AddEditControlToEditQend(eq,&ec,ma);

    PrepareEditControl(&ec);
    CutWSTR(jews, 0);
    wsAppendChar(jews, jid_set.jid_ask?0xE116:0xE117);
    ConstructEditControl(&ec,9,0x40,jews,jews->wsbody[0]); 
  }
  else
  {
    ascii2ws(jews, "Удалить контакт:");
    ConstructEditControl(&ec,1,0x40,jews,256);      
    AddEditControlToEditQend(eq,&ec,ma);
