...
TRESOURCE* ActiveContact = 0;

extern char logmsg[512];
extern char My_Presence;
extern const char* PRESENCES[12];
...
void CList_RedrawCList()
{
  ...
  int font_y = GetFontYSIZE(CLIST_FONT)+2; 
  int scr_w=ScreenW();
  char path_to_pic[128];
  N_cont_disp = sdiv(font_y,ScreenH()-5 + (24)-(24))-2;
  if(!cltop)return;
  ...
  while(ClEx)
  {
    if(ClEx->ResourceCount)
    {
      ...
      while(resEx)
      {
        ...
        if((i>(Active_page-1)*N_cont_disp) && ((Display_Offline  |  resEx->status!=6 | resEx->has_unread_msg) && Is_Right_Vis_Mode))
        {
          if(i==CursorPos)
          {
            lineColor=CURSOR;
          ...
          start_y = 5 + (24) + (i - (Active_page-1)*N_cont_disp)*font_y;
          ...
          DrawString(out_ws,Roster_getIconWidth(path_to_pic)+2,start_y+2,scr_w-1,start_y+font_y,CLIST_FONT,0,(char *)(&(fcolor)),0);

          Alternation=!Alternation; 
        }
        if((Display_Offline  |  resEx->status!=6 | resEx->has_unread_msg) && Is_Right_Vis_Mode) i++;
        resEx = resEx->next;
...
unsigned int CList_GetNumberOfOnlineUsers()
{
  ...
  while(ClEx)
  {
    ...
    while(ResEx)
    {
      if((ResEx->status!=6 || ResEx->has_unread_msg) && (ClEx->IsVisible==1 || ResEx->entry_type==T_GROUP || ResEx->entry_type==T_CONF_ROOT))
        Online++;
      ResEx=ResEx->next;
    }
    ClEx = ClEx->next;
...
unsigned int CList_GetNumberOfUsers_Visible()
{
  ...
  while(ClEx)
  {
    while(ClEx && (CList_isGroup(ClEx) || CList_isMUC(ClEx)) && ClEx->IsVisible == 0) 
    {
...
void CList_AddSystemMessage(char* jid, char status, char* status_msg)
{
  ...
  switch (status)
  {
  case 8:   strcpy(sws,"Пришел запрос на авторизацию"); break;
  case 9:  strcpy(sws,"Авторизация дана!"); break;
  case 10: strcpy(sws,"Авторизация удалена!"); break;
  case 11:strcpy(sws,"В авторизации отказано"); break;
  }
...
void CList_AddMessage(char* jid, MESS_TYPE mtype, char* mtext)
{
  ...
  char IsMe = strstr(mtext,"/me ")==mtext ? 1 : 0; 
  if(mtype==MSG_ME)
  {
  ...
  if(!contEx)
  {
    ...
    mfree(qjid);
    CList_AddResourceWithPresence(jid, 6, 0,0);
  }
...
void MoveCursorTo(TRESOURCE* NewResEx)
{
  ...
  while(ClEx)
  {
    while((CList_isGroup(ClEx) || CList_isMUC(ClEx)) && ClEx->IsVisible == 0) 
    {
      char c_group = ClEx->group;
      if (c_group & 0x80) 
      {
    ...
    while(ResEx)
    {
      ...
      if (Display_Offline || ResEx->status!=6)
        pos++;
      ResEx=ResEx->next;
    }
    ClEx = ClEx->next;
