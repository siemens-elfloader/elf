...
SASL_AUTH_DATA SASL_Auth_data = {0, 0, 0, 0, 0};
...
void Use_Plain_Auth_Report()
{
  ...
  char *fin = malloc(strlen(Data_To_Send) + strlen(s) + 1 - 2); 
...
void Decode_Challenge(char *challenge)
{
  ...
  int unk5 = 0;
  Base64Decode(challenge,strlen(challenge), decoded_challenge, 1024, 0, &unk5);  
...
void Process_Auth_Answer(char *challenge, char sendanswer)
{
  ...
  int unk5 = 0;
  Base64Decode(challenge,strlen(challenge), decoded_challenge, 256, 0, &unk5);  
...
void SASL_Process_Error(XMLNode *nodeEx)
{
  ...
  if(err)
  {
    ...
    if(!strcmp(err->name, "not-authorized"))strcat(logmsg, "\nBad password");
  }
}
