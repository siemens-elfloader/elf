...
void Add_vCard_Value(VCARD dest, char *par_name, char *val)
{
  ...
  while (field_id<52 && strcmp(par_name, vcard_index[field_id]))
    field_id++;
...
void DecodePhoto(char *path, void *data)
{
  ...
  binlen = Base64Decode(data, datlen, buf, datlen, 0, &unk5);
  ...
  unlink(path, &ec);
  ec=0;   
  volatile int f = fopen(path, 2 +0x100+ 8 + 0x8000, 0x80+ 0x100, &ec);
  if(!ec)
  {
...
int vcinfo_onkey(GUI *gui, GUI_MSG *msg)
{
  ...
  if (desc)
  {
    if (msg->gbsmsg->msg==0x193 && msg->gbsmsg->submess==0x1A && EDIT_GetFocus(gui)==desc->update_pos)
    {
    ...
    if (msg->gbsmsg->msg==0x193 && msg->gbsmsg->submess==0x1A && EDIT_GetFocus(gui)==desc->photo_show_pos)
    {
      CLIST *ClEx = CList_FindContactByJID(desc->jid);
      if (!ClEx)
        return 0;
    ...
    if (msg->gbsmsg->msg==0x193 && msg->gbsmsg->submess==0x1A && EDIT_GetFocus(gui)==desc->photo_logo_pos)
    {
      CLIST *ClEx = CList_FindContactByJID(desc->jid);
      if (!ClEx)
        return 0;
...
HEADER_DESC vcinfo_hdr={0,0,0,0,0,(int)"vCard",0x7FFFFFFF};
  ...
  if (vcard)
  {
    ...
    for (int i=0; i<52; i++)
    {
      ...
      if (strcmp(vcard_index[i], "!PHOTO") && strcmp(vcard_index[i], "!LOGO"))
      {
        ...
        ConstructEditControl(&ec, 1, 0x40, ws_info, 1024);
        AddEditControlToEditQend(eq,&ec,ma);
        utf8_2ws(ws_info, vcard[i], 1024);
        ConstructEditControl(&ec, 3, 0x40, ws_info, 1024);
        AddEditControlToEditQend(eq,&ec,ma);
      }
    }
    ...
    {
      ascii2ws(ws_info, "Показать аватару");
      ConstructEditControl(&ec, 9, 0x40, ws_info, 1024);
    ...
    {
      ascii2ws(ws_info, "Показать логотип");
      ConstructEditControl(&ec, 9, 0x40, ws_info, 1024);
