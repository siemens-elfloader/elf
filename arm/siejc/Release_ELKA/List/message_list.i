  ...
  if(msg->gbsmsg->submess==0x0B)
  {
    ...
    if(ec.pWS->wsbody[0])
    {
      ...
      char message_str[]="/message ";
        if(strstr(body, message_str)==body)  
        {
      ...
      if(!is_gchat)
      {
        if(strstr(body, attention_str)==body)  
        {
          is_attention = 1;
        }
      ...
      else
      {
        if(strstr(body, part_str)==body)  
        {
          ...
          Send_Leave_Conference(room->JID, strlen(body)>strlen(part_str) ? (char*)(body+strlen(part_str)+1):0);
          Mess_was_sent = 1;
          mfree(body);
          FreeWS(ws);
          return 1;
        }
        if(strstr(body, topic_str)==body)  
        {
          CLIST* room=CList_FindContactByJID(CList_GetActiveContact()->full_name);
...
HEADER_DESC inp_hdr={0,0,0,0,0,(int)"Новое...",0x7FFFFFFF};
...
void Init_Message(TRESOURCE* ContEx, char *init_text)
{
  ...
  ConstructEditControl(&ec, 3, 0x40 | ((FIRST_LETTER)?0x1000:0), ws, 4096);
  AddEditControlToEditQend(eq,&ec,ma);
  edmessage_id=CreateInputTextDialog(&inp_desc,&inp_hdr,eq,1,0);
  FreeWS(ws);
}

DISP_MESSAGE* MessagesList = 0;      
...
void mGUI_onRedraw(GUI *data)
{
  ...
  DrawRectangle(0,(24),ScreenW()-1,(24)+FontSize*2+1,0,
                   0, (char *)(&(MESSAGEWIN_TITLE_BGCOLOR)));

  DrawRectangle(0,(24)+FontSize+2,ScreenW()-1,ScreenH()-1,0,
                   0, (char *)(&(MESSAGEWIN_BGCOLOR)));
  ...
  DrawString(ws_title,1,(24)+1,ScreenW()-1,(24)+FontSize+1,MESSAGEWIN_FONT,0,(char *)(&(MESSAGEWIN_TITLE_FONT)),0);
  ...
  while(ml)
  {
    if((i_ctrl>(CurrentPage-1)*lines_on_page) && (i_ctrl<=CurrentPage*lines_on_page))
    {
      ...
      DrawRectangle(0, (24) + FontSize + 2 + Y_pos, ScreenW() - 1, (24) + FontSize + 2 + ml->line_height + Y_pos - kur, 0,
                       (char *)(&(MsgBg2Color)), (char *)(&(MsgBgColor)));

      DrawString(ml->mess, 1, (24) + FontSize + 2 + Y_pos, 
                ScreenW()-1, (24) + FontSize + 2 + ml->line_height + Y_pos,
                ml->mtype==MSG_NICKGCHAT?9:MESSAGEWIN_FONT, 0,
                (char *)(&(MESSAGEWIN_CHAT_FONT)), 0);
...
void ParseMessagesIntoList(TRESOURCE* ContEx)
{
  ...
  int screen_width = ScreenW() - 1 - 1 ;

  int line_width = 0; 
  int line_height = GetFontYSIZE(MESSAGEWIN_FONT); 
  int symbol_width = 0;  
  ...
  while(MessEx)
  {
    if(parsed_counter>=OLD_MessList_Count)
    {
      ...
      if (Is_Smiles_Enabled && SmilesImgList)
      {
	CharsToSmilesUNI(temp_ws_1, MessEx->mess); 
      }
      else
      ...
      for(int i = 1; i <= temp_ws_1->wsbody[0]; i++)
      {
        ...
        if ((symb==0x000A) || (symb==0x000D) || (symb==0x00A0)) 
	{
	L_ADD:
	...
	{
          if(MessEx->mtype==MSG_NICKGCHAT)symbol_width = GetSymbolWidth(symb,9);
	  ...
	  if (line_width > screen_width)
	  {
	    ...
	    goto L_ADD; 
          ...
          if (Is_Smiles_Enabled && SmilesImgList)
          {
            if (symb >= 0xE12A && symb <= 0xE12A + smiles_loaded) 
              if (line_height < symbol_width) 
                line_height = symbol_width; 
          }
	  wsAppendChar(temp_ws_2, symb);
	  cnt ++;
	  if (i == temp_ws_1->wsbody[0]) goto L_ADD; 
	  if (line_width == screen_width) goto L_ADD; 
	}
      }
      FreeWS(temp_ws_1);
      FreeWS(temp_ws_2);
...
void Display_Message_List(TRESOURCE* ContEx)
{
  ...
  lines_on_page = sdiv(FontSize, ScreenH() - 14 - (24));
  ParseMessagesIntoList(Resource_Ex);
  Calc_Pages_Data();
  CurrentPage = MaxPages;
  OLD_MessList_Count = Resource_Ex->total_msg_count;
