...
void AboutDlg(void)
{
  ...
  snprintf(msg,l,msg_tpl, VERSION_VERS, 3006, "Feb 23 2011");
  ShowMSG(0,(int)msg);
  mfree(msg);
};

char MMenu_Contents[15-1];
int cmMS_ICONS[15+1];
int MainMenu_ID;


HEADER_DESC menuhdr={0,0,131,21,0,(int)"Меню",0x7FFFFFFF};
...
int mmenu_keyhook(void *data, GUI_MSG *msg)
{
  ...
  if(msg->keys==0x18 || msg->keys==0x3D)
  {
	...
	Is_Vibra_Enabled=!(Is_Vibra_Enabled);
        cmMS_ICONS[6]=(int)icon_array[Is_Vibra_Enabled];
	RefreshGUI();
      break;
      }
    case 7:
	...
	Is_Sounds_Enabled=!(Is_Sounds_Enabled);
        cmMS_ICONS[7]=(int)icon_array[Is_Sounds_Enabled];
	RefreshGUI();
      break;
      }
    case 8:
	...
	CList_ToggleOfflineDisplay();
        cmMS_ICONS[8]=(int)icon_array[Display_Offline];
	RefreshGUI();
      break;
      }
    case 10:
      {
        ...
        cmMS_ICONS[10]=(int)icon_array[Is_Autostatus_Enabled];
	RefreshGUI();
	break;
	}
    case 11:
    ...
    case 15:
      {
        ...
        cmMS_ICONS[15]=(int)icon_array[Is_Smiles_Enabled];
	RefreshGUI();
        break;
      }
    case 9:
...
void InitMMenuArray()
{
  for(int i=0;i<15;i++)MMenu_Contents[i]=0;
}
...
void menuitemhandler(void *data, int curitem, void *unk)
{
  ...
  switch(MMenu_Contents[curitem])
  {
    case 1:
      {
      strcpy(test_str,"Контакт");
        break;
      }
    case 2:
      {
      strcpy(test_str,"Статус");
        break;
      }
    ...
    case 6:
      {
      strcpy(test_str,"Режим вибры");
        break;
      }
    case 7:
      {
      strcpy(test_str,"Режим звука");
        break;
      }
  case 8:
      {
      strcpy(test_str,"Офф. контакты");
        break;
      }
    case 10:
      {
      strcpy(test_str,"Автостатус");
        break;
      }
    case 11:
      {
      strcpy(test_str,"Настройки");
        break;
      }
    case 12:
      {
      strcpy(test_str,"Цвета");
        break;
      }
    case 13:
      {
      strcpy(test_str,"Об эльфе...");
        break;
      }
    case 14:
      {
      strcpy(test_str,"Выход");
        break;
      }
    case 15:
      {
      strcpy(test_str,"Смайлы");
        break;
      }
    case 9:
      {
      strcpy(test_str,"Настроения");
        break;
      }
  }
...
void Init_MMIcon_array()
{
  ...
  for(int i=0;i<=15;i++)cmMS_ICONS[i]=0;

  icon_array[1]=GetPicNByUnicodeSymbol(0xE116);
  icon_array[0]=GetPicNByUnicodeSymbol(0xE117);
  cmMS_ICONS[6]=(int)icon_array[Is_Vibra_Enabled];
  cmMS_ICONS[7]=(int)icon_array[Is_Sounds_Enabled];
  cmMS_ICONS[8]=(int)icon_array[Display_Offline];
  cmMS_ICONS[10]=(int)icon_array[Is_Autostatus_Enabled];
  ...
  cmMS_ICONS[1]=(int)ICON_INFO;
  cmMS_ICONS[2]=(int)ICON_STATUS;
 
 
  
  cmMS_ICONS[11]=(int)ICON_SETTING;
  cmMS_ICONS[12]=(int)ICON_COLOR;
  cmMS_ICONS[13]=(int)ICON_ABOUT;
  cmMS_ICONS[14]=(int)ICON_EXIT;
  cmMS_ICONS[9]=(int)ICON_STATUS;
}
...
void MM_Show()
{
  ...
  {  
  MMenu_Contents[n_items++]=1;
  MMenu_Contents[n_items++]=2;
  MMenu_Contents[n_items++]=9;
  
 
 
  }
  MMenu_Contents[n_items++]=6;
  MMenu_Contents[n_items++]=7;
  MMenu_Contents[n_items++]=8;
  if(AUTOSTATUS_ENABLED) MMenu_Contents[n_items++]=10;
  MMenu_Contents[n_items++]=15;
  MMenu_Contents[n_items++]=11;
  MMenu_Contents[n_items++]=12;
  MMenu_Contents[n_items++]=13;
  MMenu_Contents[n_items++]=14;

  if(n_items)
  {
    patch_rect(&menuhdr.rc,0,(24),ScreenW()-1,HeaderH()+(24));
