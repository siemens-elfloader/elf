...
void _getbookmarkslist()
{
  ...
  char xmlns_t[]="xmlns";
  XMLNode *xml_bookmark = XML_CreateNode(storage_t, 0);
  XML_Set_Attr_Value(xml_bookmark, xmlns_t, "storage:bookmarks");
  char gget[]= "get";
  char iqv[]= "jabber:iq:private";
  SendIq(0, gget, priv_id, iqv, xml_bookmark);
}
...
HEADER_DESC bm_menuhdr={0,0,0,0,0,(int)bmmenu_header,0x7FFFFFFF};
...
MENU_DESC bm_menu=
{
  8,bm_menu_onkey,bm_menu_ghook,0,
  bmmenusoftkeys,
  &bmmenu_skt,
  0x11,
  bm_menu_iconhndl,
...
int bm_menu_onkey(void *data, GUI_MSG *msg)
{
  ...
  if(msg->keys==0x18 || msg->keys==0x3D)
  {
    ...
    if(it->mucname && it->nick && (msg->keys == 0x3D)) 
    {
      extern const unsigned int DEFAULT_MUC_MSGCOUNT;
...
void Disp_BM_Menu()
{
  if(BM_ROOT)
  {
    ...
    patch_rect(&bm_menuhdr.rc,0,(24),ScreenW()-1,HeaderH()+(24));
