...
void init_turnoff()
{
  ...
  sprintf(shutdown2, "%s%s", turnoff_path, "shutdown_logo.png");
...
void DrawProgress(WSHDR *dat, int casi)
{
  if (casi)
  {
    ...
    DrawScrollString(data4,18,97,100,97+GetFontYSIZE(11),1,11,TEXT_ALIGNLEFT,ctasks,GetPaletteAdrByColorIndex(23));
    wsprintf(data4,"[OK]");
...
/*    void *canv = BuildCanvas();
    ...
    DrawRectangle(17,109,17+Get_WS_width(dat,11),109+GetFontYSIZE(11),1);
    else DrawRectangle(17,109,87,109+GetFontYSIZE(11),1);*/
    DrawScrollString(dat,18,109,87,109+GetFontYSIZE(11),1,11,TEXT_ALIGNLEFT,ctasks,GetPaletteAdrByColorIndex(23)); 
  }
  else
  {
    if(shutd) wsprintf(data4,"Shut down......");
    else wsprintf(data4,"Restarting......");
    DrawString(data4,18,109,115,109+GetFontYSIZE(11)+1,11,0,ctasks,GetPaletteAdrByColorIndex(23));
  }
...
void task_list3()
{
  ...
  DrawImg(23, 60, (int)shutdown2);
  DrawImg(68, 60, (int)shutdown2);
...
/*      if(nl->is_daemon)
      ...
      {*/
        ...
        {
          if(nl->is_daemon) DrawString(nl->name,18,i+1,115,i+GetFontYSIZE(11),11,0,cdaemons,GetPaletteAdrByColorIndex(23));
          else
        DrawString(nl->name,18,i+1,115,i+GetFontYSIZE(11),11,0,ctasks,GetPaletteAdrByColorIndex(23));
        }
        else
        {
          if(pos!=show)
          {
            if(nl->is_daemon) DrawScrollString(nl->name,18,i+1,100,i+GetFontYSIZE(11),1,11,TEXT_ALIGNLEFT,cdaemons,GetPaletteAdrByColorIndex(23)); else
          DrawScrollString(nl->name,18,i+1,100,i+GetFontYSIZE(11),1,11,TEXT_ALIGNLEFT,ctasks,GetPaletteAdrByColorIndex(23));
          }
          else
          {
            if(nl->is_daemon) DrawScrollString(nl->name,18,i+1,100,i+GetFontYSIZE(11),sm+1,11,TEXT_ALIGNLEFT,cdaemons,GetPaletteAdrByColorIndex(23)); else
            DrawScrollString(nl->name,18,i+1,100,i+GetFontYSIZE(11),sm+1,11,TEXT_ALIGNLEFT,ctasks,GetPaletteAdrByColorIndex(23));
            refresh=30;
...
//      DrawImg(100,39,(int)selection_img);
      DrawImg(23, 60, (int)shutdown2);
      DrawImg(68, 60, (int)shutdown2);
      if(shutd) DrawImg(16,91,(int)shutdown1);
      else
...
int my_keyhook(int submsg, int msg)
{
  ...
  if ((submsg==RED_BUTTON)&&(IsIdleUiOnTop()&&(use_idle)))
  {
    ShowMenu(0);
