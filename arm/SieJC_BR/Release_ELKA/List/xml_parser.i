...
void SaveTree(XMLNode *tree){
  ...
  f=fopen(savepath, 2+0x100+8+0x8000,0x80+0x100,&ul);
  SaveBranch(XMLTree);
  fclose(f,&ul);
}
 
void EndTag(void){
  if((TagState == 2)||(TagState == 4)){
	...
	while ((i<size)){
		...
		case 5:
			if (IsLit (c)){
				...
				MSState = 11;
				EndTag();
			}else if (!IsSpace(c) && (c != '?' && TagState != 5)){
				Finish();
				return XMLTree;
		...
		case 11:
			...
			if (c == '<'){
                          ...
                          if((TagState == 2)) strcat(Text,sh_str);
                        }
			break;
		}
            i++;
...
XMLNode* XML_Get_Child_Node_By_Name_And_Attr(XMLNode* node, char* req_node_name, char* req_attr_name, char* req_attr_velue)
{
  ...
  {
    if(nodeEx->name)
      if(!strcmp(req_node_name, nodeEx->name))    
      {
