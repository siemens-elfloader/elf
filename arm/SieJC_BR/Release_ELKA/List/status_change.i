...
static const char * const status_texts[7]=
{
  DEFTEX_ONLINE,
  DEFTEX_CHAT,
  DEFTEX_AWAY,
...
HEADER_DESC select_status_menu_header={0,0,131,21,0,(int)"Выбор статуса",0x7FFFFFFF};

int select_status_menu_softkeys[]={0,1,2};

SOFTKEY_DESC select_status_menu_sk[]=
{
  {0x0018,0x0000,(int)"Изменить"},
...
int select_status_menu_icons[7];
...
static const char * const select_status_menu_texts[7]=
{
  "Онлайн",
  "Готов болтать",
  "Отсутствую",
...
void ShowStatusSelectMenu()
{
  select_status_menu_icons[0] = (int)Roster_getIconByStatus(0, 0);
  select_status_menu_icons[1] = (int)Roster_getIconByStatus(0, 1);
  select_status_menu_icons[2] = (int)Roster_getIconByStatus(0, 2);
  select_status_menu_icons[3] = (int)Roster_getIconByStatus(0, 3);
  select_status_menu_icons[4] = (int)Roster_getIconByStatus(0, 4);
  select_status_menu_icons[5] = (int)Roster_getIconByStatus(0, 5);
  select_status_menu_icons[6] = (int)Roster_getIconByStatus(0, 6);
  if (My_Presence < 6)
    select_status_menu_header.icon = &select_status_menu_icons[My_Presence];
  
  patch_header(&select_status_menu_header);
  CreateMultiLinesMenu(0, 0, &select_status_menu_struct, &select_status_menu_header, My_Presence < 6 ? My_Presence: 0, 7);
}
...
{
  EDITCONTROL ec;
  if(msg->gbsmsg->submess==0x0B  || msg->keys == 0x18)
  {
    ...
    pr_info->message = (status_text) ? Mask_Special_Syms(status_text) : 0;
    SUBPROC((void*)Send_Presence,pr_info);
...
HEADER_DESC edit_status_menu_header={0,0,131,21,0,(int)"Статус",0x7FFFFFFF};
...
void EditStatus(int status_n)
{
  ...
  ascii2ws(ws, "Введите текст статуса:");
  ConstructEditControl(&ec, 1, 0x40, ws, ws->wsbody[0]);
  AddEditControlToEditQend(eq,&ec,ma);

  ascii2ws(ws, status_texts[status_n]);
  ConstructEditControl(&ec, 3, 0x40, ws, 256);
  AddEditControlToEditQend(eq, &ec, ma);

  ascii2ws(ws, "Приоритет:");
  ConstructEditControl(&ec, 1, 0x40, ws, ws->wsbody[0]);
  AddEditControlToEditQend(eq, &ec, ma);
  
  extern ONLINEINFO OnlineInfo;
  wsprintf(ws, "%d",OnlineInfo.priority);
  ConstructEditControl(&ec, 5, 0x40, ws, 2);
  AddEditControlToEditQend(eq, &ec, ma);
