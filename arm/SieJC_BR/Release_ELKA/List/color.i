...
int ReadColor(const char * fname)
{
  ...
  if (GetFileStats(color_file, &fstat, &io_error) != -1)
  {
    if (!io_error)
    {
      if ((hFile = fopen(color_file, 0 + 0x8000, 0x80, &io_error)) != -1)
      {
        if (!io_error)
        {
...
HEADER_DESC select_color_menu_header = {0, 0, 0, 0, 0, (int)"Цвета", 0x7FFFFFFF};

MENU_DESC select_color_menu_struct=
{
  8, select_color_menu_onkey, select_color_menu_ghook, 0,
