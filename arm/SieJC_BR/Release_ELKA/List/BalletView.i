...
void debugv(char *file,int line,void *p, int sz)
{
  if ((debug_file=fopen("0:\\zbin\\siejc\\debug.txt",2+0x100+debugA,0x80+0x100,&debug_ul))!=-1)
  {
...
int GetFontHeight(int font, int atribute)
{
  ...
  if (atribute&16)   height+=1;
  if (atribute&8)   height+=1;
  if (atribute&32)  height+=2;
  return height;
}
unsigned int SearchNextDisplayLine(VIEWDATA *vd, LINECACHE *p, unsigned int *max_h)
{
  ...
  while(pos<vd->rawtext_size)
  {
    ...
    {
      h=GetFontHeight(p->bold?9:8,p->underline?8:0+p->ref?16:0);
    }
    switch(c)
    {
    case 0x0A:
    ...
    cw=GetSymbolWidth(c,p->bold?9:8);
    left-=cw;
    if (left<0)
    {
      int b=1;
      ...
      if (p->ref)
      {
        while (1)
        {
          if (vd->rawtext[pos-b]==(0xE005)) break;
          if (vd->rawtext[pos-b]==0x000A)
          {
            b++;
            break;
          }
          ...
          cw+=GetSymbolWidth(vd->rawtext[pos-b],p->bold?9:8);
          b++;
        }
      }
      else
    ...
    if (cw>=ScreenW()/2||h>GetFontHeight(8,0)*2)
    {
      if (left+cw<ScreenW())
      {
        p->centerAtAll=1;
        if (vd->rawtext[pos-1]==(0xE005))
        {
          return pos-2;
        }
        else
...
{
  if ((vd->lines_cache_size%256)==0)
  {
    
    vd->lines_cache=realloc(vd->lines_cache,(vd->lines_cache_size+256)*sizeof(LINECACHE));
  }
  ...
  while(ypos<=scr_h)
  {
    ...
    if (lcheck<=1&&vd->lines_cache_size>1)
    {
      ...
      while(len>0&&(dc<32000))
      {
        ...
        if (c==(0xE01C)||c==(0xE01D)||c==(0xE01E)||             c==(0xE01F)||c==0x000A)
        {
          goto L1;
        }
        
        if ((c==(0xE007))||(c==(0xE006)))
        {
          len--;
          sc++; sc++; 
        ...
        ws->wsbody[++dc]=c;
        ws_width+=GetSymbolWidth(c,lc->bold?9:8);
      ...
      if (do_draw&&y2!=ypos)
      {
        ...
        if ((rcs_work.x!=0||rcs_work.x2!=0)&&rcs_work.x!=rcs_work.x2)
        {
	      ...
	      DrawString(ws,x,ypos,scr_w,y2,
		               lc->bold?9:8,
                   128+(lc->underline?8:0),
		               def_ink,GetPaletteAdrByColorIndex(23));
        
        if (lcheck) vd->Resource_Ex->has_unread_msg =0; 
      }
      if (y2>=scr_h||lcheck)
      {
        if (rcs_num)
        {
          char *c1=(char *)(&(MESSAGEWIN_CURSOR_BGCOLOR));
...
void VIEWDATA_onCreate(VIEW_GUI *data, void *(*malloc_adr)(int))
{
  ...
  data->vd.pos_cur_ref=0xFFFFFFFF; 
}
...
const void * const VIEWDATA_methods[11]={
  (void *)VIEWDATA_onRedraw,	
  (void *)VIEWDATA_onCreate,	
  (void *)VIEWDATA_onClose,	  
  (void *)VIEWDATA_onFocus,	  
  (void *)VIEWDATA_onUnfocus,	
  (void *)VIEWDATA_onKey,	    
  0,
  (void *)kill_data,          
...
void RawInsertChar(VIEWDATA *vd, int wchar)
{
  if ((vd->rawtext_size%(16384))==0)
  {
    
    vd->rawtext=realloc(vd->rawtext,(vd->rawtext_size+(16384))*2);
  }
...
void AddEndRef(VIEWDATA *vd)
{
  ...
  REFCACHE *p;
  if ((vd->ref_cache_size%(256))==0)
  {
    vd->ref_cache=realloc(vd->ref_cache,(vd->ref_cache_size+(256))*sizeof(REFCACHE));
  }
...
OMS_DYNPNGLIST *AddToDPngQueue(VIEWDATA *vd, IMGHDR *img, int is_index)
{
  ...
  if (!dpl)
  {
    odp->dp.icon=GetPicNByUnicodeSymbol((wchar=0xE12A));
    odp->w_char=wchar;
