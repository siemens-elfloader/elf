...
void Send_Auth()
{
  ...
  SendIq(0, "set", auth_id, "jabber:iq:auth", payload);
  mfree(payload);
  LockSched();
...
char *Generate_Caps()
{
  ...
  sprintf(answer, "client/mobile//SieJC %s:%i"
    ...
    "<%s"
    "<%s"
    "<%s", VERSION_VERS, 2774, "http://jabber.org/protocol/disco#info", "http://jabber.org/protocol/muc", "jabber:iq:last", "jabber:iq:time", "jabber:iq:version", "urn:xmpp:ping");
...
void Send_Presence(PRESENCE_INFO *pr_info)
{
  ...
  if(pr_info->status!=5)
  {
    if(pr_info->message)
    {
      ...
      snprintf(presence,1024,presence_template, pr_info->priority, PRESENCES[pr_info->status], pr_info->message, VERSION_NAME, VERSION_VERS, 2774, caps);
    }
    else
    {
      ...
      snprintf(presence,1024,presence_template, pr_info->priority, PRESENCES[pr_info->status], VERSION_NAME, VERSION_VERS, 2774, caps);
    }
  }
  else
  {
  ...
  while(m_ex)
  {
    ...
    if(pr_info->message)
    {
      ...
      snprintf(presence,1024,presence_template, _from, _to, PRESENCES[pr_info->status], pr_info->message, VERSION_NAME, VERSION_VERS, 2774, caps);
    }
    else
    {
      ...
      snprintf(presence,1024,presence_template, _from, _to, PRESENCES[pr_info->status], VERSION_NAME, VERSION_VERS, 2774, caps);
    }
    mfree(_to);
    mfree(_from);
    SendAnswer(presence);
...
void SendMessage(char* jid, IPC_MESSAGE_S *mess)
{
  ...
  char* msg_buf = malloc(4096*2+200);
  if(mess->IsGroupChat)
  {
     sprintf(msg_buf, mes_template, _jid, m_num, "groupchat", mess->body);
  }else sprintf(msg_buf, mes_template, _jid, m_num, "chat", mess->body);
  if (DELIVERY_EVENTS && !mess->IsGroupChat) strcat(msg_buf,"<request xmlns='urn:xmpp:receipts'/>"); 
...
void SendComposing(char* jid, IPC_MESSAGE_S *mess)
{
  ...
  if(mess->IsGroupChat)
  {
     sprintf(msg_buf, mes_template, _jid, m_num, "groupchat");
  }else sprintf(msg_buf, mes_template, _jid, m_num, "chat");
...
void Report_VersionInfo(char* id, char *to)
{
  ...
  sprintf(answer, "<name>%s</name><version>%s-r%d (%s)</version><os>SIE-%s/%s %s</os>", VERSION_NAME, VERSION_VERS, 2774, CMP_DATE, ph_model, ph_sw, OS);
  SendIq(to, "result", id, "jabber:iq:version", answer);
...
void Report_TimeInfo(char* id, char *to)
{
  ...
  SendIq(to, "result", id, "jabber:iq:time", answer);
  mfree(id);
  mfree(to);
};
...
void Report_DiscoInfo(char* id, char *to)
{
  ...
  sprintf(answer, "<identity category='client' type='phone'/>"
                    ...
                    "<feature var='%s'/>"
                    "<feature var='%s'/>"
                    "<feature var='%s'/>", "http://jabber.org/protocol/disco#info", "jabber:iq:version", "http://jabber.org/protocol/muc", "jabber:iq:time", "jabber:iq:last","urn:xmpp:ping");
  if(DELIVERY_EVENTS)
  {
  ...
  if(USE_ATTENTION)
  {
    char xevents_feature[]="<feature var='" "urn:xmpp:attention:0" "'/>";
    strcat(answer, xevents_feature);
  }
  ...
  SendIq(to, "result", id, "http://jabber.org/protocol/disco#info", answer);
...
void _leaveconference(char *conf_jid)
{
  ...
  mfree(msg);
  mfree(pr);
  } else Send_ShortPresence(conf_jid,6);
  mfree(conf_jid);
}
...
if(!strcmp(gerr,iqtype)) 
{
  ...
  if(errcode)
  {
    ...
    switch (errcode_n)
      ...
      case 404: sprintf(logmsg,"Not fonud"); break;
...
static char r[512];       
    ...
    if(x_node = XML_Get_Child_Node_By_Name_And_Attr(node,loc_x, loc_xmlns, "http://jabber.org/protocol/muc#user")) 
    {
      ...
      if (!CList_FindMUCByJID(Conference->JID)) return; 
      ...
      if(sstatus)  
      {
        ...
        switch (errcode_n)
        {
         ...
         case 201: sprintf(r, "New MUC crated. Owner-%s", nick); break; 
