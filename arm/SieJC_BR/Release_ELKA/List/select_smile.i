...
int RenderPageSm(SMILE_GUI *data, int is_draw)   
{
  int scr_w=ScreenW()-1;
  int scr_h=ScreenH()-1;
  int res=0;
      ...
      if (is_draw)
      {
        if (i==data->cur_pos_y && k==data->cur_pos_x)
        {
          DrawRectangle(x - 1, y2 - 1, x + img->w, y2 + img->h, 0,
                        (char *)(&(CURSOR)), (char *)(&(CURSOR_BORDER)));
        }
        DrwImg(img,  x,y2);
      }
      x += img->w + 1;
  ...
  if (is_draw)
  {
    ...
    if (sm)
    {
      ...
      DrawString(data->ws,1,y+1,scr_w,y+1+font_size,font,0, (char *)(&(CLIST_F_COLOR_0)), GetPaletteAdrByColorIndex(23));
    }
  }
  return (res);  
}
