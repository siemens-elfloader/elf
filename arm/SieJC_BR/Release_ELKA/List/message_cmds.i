...
int LoadTemplates_new(void)
{
   ...
   while(ResList)
    {
      if((ResList->name)&&(ResList->status!=6)) 
      {
...
void UpdateTemplatesMenu_header(void)
{
  ...
  case 2:
    ...
    break;
  case 3:
    strcpy(clm_hdr_text,"Ресурсы...");
    break;
  }
}

HEADER_DESC tmpl_menuhdr={0,0,131,21,0,(int)clm_hdr_text,0x7FFFFFFF};
...
{
  void *data=FindGUIbyId(edmessage_id,0);
...
{
  8,tmpl_menu_keyhook,tmpl_menu_ghook,0,
  tmpl_menusoftkeys,
  &tmpl_menu_skt,
  0x10,
...
void select_smile(GUI * data)
{
  Mode = 0;
  void * ed_gui = 0;
  if (ed_gui = FindGUIbyId(edmessage_id, 0))
...
HEADER_DESC sel_menu_header={0,0,131,21,0,(int)"Выбор...",0x7FFFFFFF};

int sel_menu_softkeys[]={0,1,2};

MENUITEM_DESC sel_menu_items[5]=
{
  {0, (int)"Смайлы",         0x7FFFFFFF, 0, 0, 3, 0x5A2},
  {0, (int)"Команды",      0x7FFFFFFF, 0, 0, 3, 0x5A2},
  {0, (int)"Шаблоны сообщений",   0x7FFFFFFF, 0, 0, 3, 0x5A2},
  {0, (int)"Ники",  0x7FFFFFFF, 0, 0, 3, 0x5A2},
  {0, (int)"Закрыть диалог",      0x7FFFFFFF, 0, 0, 3, 0x5A2},
};

const MENUPROCS_DESC sel_menu_procs[5]=
{
  select_smile,
  select_command,
  select_template,
...
void DispSelectMenu()
{
  ...
  Select_Menu_ID = CreateMenu(0, 0, &sel_menu_struct, &sel_menu_header, 0, 5, 0, to_remove);
}
