...
{
  EDITCONTROL ec;
  static SOFTKEY_DESC mmmmsk={0x0018, 0x0000,(int)"Войти"};
...
HEADER_DESC med1_hdr={0,0,131,21,0,(int)"Вход в MUC",0x7FFFFFFF};
...
void Disp_MUC_Enter_Dialog(char* muc, char* nick, char* pass)
{
  ...
  ConstructEditControl(&ec, 3, 0x40, ws, 80);     
  AddEditControlToEditQend(eq,&ec,ma);  
  ...
  ascii2ws(ws, "Получить сообщений:");
  ConstructEditControl(&ec, 1, 0x40, ws, ws->wsbody[0]);      
  AddEditControlToEditQend(eq,&ec,ma);  
