...
void _add2history(HIST_RECORD_INFO *info)
{
  ...
  utf8_2ws(filename_WS, info->fname, 256);  
  ws_2str(filename_WS, filename_utf8, 256); 
  hFile = fopen(filename_utf8,2 + 8 + 0x8000,0x80+0x100, &io_error);
  
  if(io_error==2) 
  {
    hFile = fopen(filename_utf8,2+0x100+ 0x8000,0x80+0x100, &io_error);
  }
  if(!io_error)
  {
