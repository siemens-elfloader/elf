      ...
      while (png_ptr->zstream.avail_in)
      {
         ...
         if (ret != Z_OK && ret != Z_STREAM_END)
         {
            ...
            if (text ==  NULL)
            {
               ...
               text = (png_charp)png_malloc(png_ptr, (png_uint_32)text_size); // JL 03/24/2002 - fixed incorrect typeconvesion
         ...
         if (!png_ptr->zstream.avail_out || ret == Z_STREAM_END)
         {
            if (text == NULL)
            {
               ...
               text = (png_charp)png_malloc(png_ptr, (png_uint_32)(text_size + 1)); // JL 03/24/2002 - fixed incorrect typeconvesion
...
#ifndef PNG_NO_CONSOLE_IO
#ifdef PNG_FLOATING_POINT_SUPPORTED
            sprintf(lgn,"wx=%f, wy=%f, rx=%f, ry=%f\n",white_x, white_y, red_x, red_y);Tlog(lgn);
            sprintf(lgn,"gx=%f, gy=%f, bx=%f, by=%f\n",
               green_x, green_y, blue_x, blue_y);Tlog(lgn);
#else
            sprintf(lgn,"wx=%ld, wy=%ld, rx=%ld, ry=%ld\n",
               int_x_white, int_y_white, int_x_red, int_y_red);Tlog(lgn);
            sprintf(lgn,"gx=%ld, gy=%ld, bx=%ld, by=%ld\n",
               int_x_green, int_y_green, int_x_blue, int_y_blue);Tlog(lgn);
#endif
...
#ifndef PNG_NO_CONSOLE_IO
#  ifdef PNG_FIXED_POINT_SUPPORTED
         sprintf(lgn,"incorrect gamma=(%d/100000)\n",(int)png_ptr->int_gamma);Tlog(lgn);
#  else
#    ifdef PNG_FLOATING_POINT_SUPPORTED
         sprintf(lgn,"incorrect gamma=%f\n",png_ptr->gamma);Tlog(lgn);
#    endif
#  endif
#endif
      }
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   profile_length = (png_uint_32)(data_length - prefix_length); // JL 03/24/2002 - fixed incorrect typeconvesion
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   data_length = (int)((slength - (entry_start - chunkdata))); // JL 03/24/2002 - fixed incorrect typeconvesion
   ...
   if (png_ptr->interlaced)
   {
      ...
      png_ptr->iwidth = (png_ptr->width +
         ...
         row_bytes = ((png_ptr->iwidth *
            (png_uint_32)png_ptr->pixel_depth + 7) >> 3) +1;
         png_ptr->irowbytes = (png_uint_32)(png_size_t)row_bytes; // JL 03/24/2002 - fixed incorrect typeconvesion
