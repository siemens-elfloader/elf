...
const char color3[4]={0x50,0x00,0x00,0x20};
const char color4[4]={0x99,0x99,0x99,0x50};
int curent=1,select_art=0,select_alb=0,select_tra=0,max_art,max_alb,max_tra;
int ToView=1;
char masyv[30000];
typedef struct
{
...
p artists[255],albums[255],tracks[1024];
const RECT Canvas3={0,0,239,319};
int GUI2;
int view_flag=0;
int numit,polit;
int yyy1,yyy2;
int xlsh,ylsh,xrsh,yrsh;
extern IMGHDR *fonn;
    ...
    case ENTER_BUTTON:case'5':
      if (ToView==1)
      {
        ...
        if (ToView==3)
        {
          ...
          {
            Message((char *)LG_CANT_CREATE_FILE);
          }
          fclose(f,&err);
