...
__arm void LoadDaemons(void)
{
  ...
  pathlen=strlen(folder_d);
  strcat(folder_d,"*.elf");
  if (FindFirstFile(&de,folder_d,&err))
  {
    do
    {
      //strcpy(name,path);
      folder_d[pathlen]=0;
      strcat(folder_d,de.file_name);
      elfload(folder_d,0,0,0);
    }
...
{
  char s[64];
  sprintf(s,"No function on lib %d(%03X)",f,f);
  ShowMSG(1,(int)s);
}
