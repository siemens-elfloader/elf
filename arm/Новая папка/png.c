...
{
  IMGHDR * img;
  extern  char folder_i[128];
  //strcpy(DEFAULT_FOLDER,folder_i);
  unsigned int i;
  char fname[256];
  PNGLIST *pl;
  PNGLIST *pl_prev;
  ...
  else
  {
    ...
    if ((pngtop.bitmap) && (pic<20000))
    {
      ...
      if ((i=*bp)&mask80)  // Есть запись в битмапе
      {
        if (i&mask40)  
        {
          char *next=strcpy_tolow(fname,folder_i); // Картинка вроде как есть на диске
	  print10(next,pic);
          img=find_png_in_cache(fname);
          if (img) return (img);
          img=create_imghdr(fname,0);   
	...
	*bp|=mask80; // Записи нет, ставим флаг что есть
	UnlockSched();
        char *next=strcpy_tolow(fname,folder_i);
	print10(next,pic);
        img=find_png_in_cache(fname);
        if (img)
        {
