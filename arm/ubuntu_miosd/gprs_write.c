...
void write_file(char *wer)
{
  ...
  unsigned int ul2;
  FSTATS stat2;
  GetFileStats("0:\\zbin\\miosd\\miosd3.file",&stat2,&ul2);
  hFile2 = fopen("0:\\zbin\\miosd\\miosd3.file",A_ReadWrite+A_Create+A_Truncate+ A_BIN,P_READ+P_WRITE, &io_error2);
  if(hFile2!=-1)
  {
...
void write_total()
{
    ...
    hFile_ex3 = fopen("0:\\zbin\\miosd\\miosd3.file",A_ReadOnly + A_BIN,P_READ, &io_error_ex3);
    if(hFile_ex3!=-1)
    {
      fclose(hFile_ex3, &io_error_ex3);
      GetFileStats("0:\\zbin\\miosd\\miosd3.file",&stat4,&ul4);
      if(stat4.size>0)
      {
        f4=fopen("0:\\zbin\\miosd\\miosd3.file",A_ReadOnly+A_BIN,P_READ,&ul4);
        fsize4=stat4.size;
