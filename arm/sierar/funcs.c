...
TYPE ReadExp(char *fname)
{
  char Buf[24];
  int handle=fopen(fname,A_BIN,A_ReadOnly,0);
  fread(handle,Buf,23,NULL);
  fclose(handle,NULL);
  if(strstr(Buf,"Rar"))return RAR;
  if(strstr(Buf,"Pk")||strstr(Buf,"PK"))return ZIP;
  if(strstr(Buf,"7z"))return SevenZip;
  return UnExp;
}*/
typedef struct
{
...
unsigned int chardos2win(unsigned int c)
{
...
void dos2win(char *to, char *from)
{
  int wslen=strlen(from);
  
  int slen=0;
  while(slen!=wslen)
  {
    *to++=chardos2win(from[slen]);
    slen++;
  }
  *to=0;
