    ...
    ISzAlloc *allocTemp)
{
  ...
  if (!TestSignatureCandidate(header)) {Tlog("wrong sign");return SZ_ERROR_NO_ARCHIVE;}
  if (header[6] != k7zMajorVersion) {Tlog("unsupported version");return SZ_ERROR_UNSUPPORTED;}
  Tlog("sign ok");
  ...
  {Tlog("crc error");return SZ_ERROR_CRC;}
  sprintf(lgn,"nextHeaderSize=%d",nextHeaderSize);Tlog(lgn);
  nextHeaderSizeT = (size_t)nextHeaderSize;
  if(nextHeaderSizeT != nextHeaderSize) {Tlog("headers size wrong");return SZ_ERROR_MEM;}
  if (nextHeaderSizeT == 0) {Tlog("header size return");return SZ_OK;}
  if(nextHeaderOffset > nextHeaderOffset + nextHeaderSize || nextHeaderOffset > nextHeaderOffset + nextHeaderSize + k7zStartHeaderSize)
  {Tlog("no archive");return SZ_ERROR_NO_ARCHIVE;}
  Tlog("point 0");
  
    Int64 pos = 0;
    RINOK(inStream->Seek(inStream, &pos, SZ_SEEK_END));
    sprintf(lgn,"end seek ok - %d",pos);Tlog(lgn);
    
    if ((UInt64)pos < nextHeaderOffset ||
        (UInt64)pos < k7zStartHeaderSize + nextHeaderOffset ||
        (UInt64)pos < k7zStartHeaderSize + nextHeaderOffset + nextHeaderSize) {Tlog("input eof");return SZ_ERROR_INPUT_EOF;}
    else Tlog("point 0.2");
  ...
  {Tlog("error mem");return SZ_ERROR_MEM;}
  Tlog("point 3");
  ...
  if (res == SZ_OK)
  {
    ...
    if (CrcCalc(buffer.data, nextHeaderSizeT) == nextHeaderCRC)
    {
      ...
      sd.Data = buffer.data;
      sd.Size = buffer.size;
      sprintf(lgn,"sd.size = %d",sd.Size);Tlog(lgn);
      res = SzReadID(&sd, &type);
      sprintf(lgn,"type=%d",type);Tlog(lgn);
      if (res == SZ_OK)
      {
        Tlog("res ok 2");
        if (type == k7zIdEncodedHeader)
        {
          Tlog("type - encoded header");
          CBuf outBuffer;
          Buf_Init(&outBuffer);
          res = SzReadAndDecodePackedStreams(inStream, &sd, &outBuffer, p->startPosAfterHeader, allocTemp);
          if(res != SZ_OK) Buf_Free(&outBuffer, allocTemp);
          else
          {
            Tlog("res ok 3");
    ...
    else Tlog("crc cannot calculate");
  }
  else {Tlog("stream read error");}
  Buf_Free(&buffer, allocTemp);
  return res;
}
