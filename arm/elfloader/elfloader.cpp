...
long elfload(char *filename, void *param1, void *param2, void *param3){
...
#ifdef wintel
  FILE *fin=NULL;
  if ((fin=fopen(filename,"rb"))==NULL) return -1;			//не открываетс€ ельф
  if (fread(&ehdr,sizeof(Elf32_Ehdr),1,fin)!=1) return -2;	        //не читаетс€ ельф
#endif
  ...
  if ((fin=fopen(filename, A_ReadOnly+A_BIN, P_READ, &iError))<0) return -1;	//не открываетс€ ельф
  if (fread(fin, &ehdr, sizeof(Elf32_Ehdr), &iError)!=sizeof(Elf32_Ehdr))	//не читаетс€ ельф
  ...
  for(n=0;n<ehdr.e_phnum;n++){
    ...
    if (fread(&(phdrs[n]),sizeof(Elf32_Phdr),1,fin)!=1) return -5;		//не читаетс€ програмный заголовок
#endif
    
    //ARM
#ifndef wintel
    if (lseek(fin, ehdr.e_phoff+n*ehdr.e_phentsize, S_SET, &iError, &iError2)!=ehdr.e_phoff+n*ehdr.e_phentsize)
    {fclose(fin, &iError); return -4;}				//не сикаетс€ програмный заголовок
    if (fread(fin, &phdrs[n], sizeof(Elf32_Phdr), &iError)!=sizeof(Elf32_Phdr))
    {fclose(fin, &iError); return -5;}				//не читаетс€ програмный заголовок
#endif
    /////////////////////////////////////////////////////
  ...
  if ((base=(char *)malloc(maxadr-minadr))==0){		//не выдел€етьс€ пам€ть под ельф
#ifndef wintel
    fclose(fin, &iError);
#endif
    return -14;
  ...
  for(n=0;n<ehdr.e_phnum;n++){ //  обход всех сегментов
    ...
    {fclose(fin, &iError); mfree(base); return -6;}		//не сикаетс€ динамический сегмент
#endif
    /////////////////////////////////////////////////////////////////////
	...
	//WINTEL
#ifdef wintel
	if (fread((void *)&base[phdrs[n].p_vaddr-minadr],phdrs[n].p_filesz,1,fin)!=1) return -11;	//не читаетс€ програмный сегмент
#endif
	
	//ARM
#ifndef wintel
	if (fread(fin, &base[phdrs[n].p_vaddr-minadr], phdrs[n].p_filesz, &iError)!= phdrs[n].p_filesz)
	{fclose(fin, &iError); mfree(base); return -11;}//не читаетс€ програмный сегмент
#endif
	///////////////////////////////////////////////////////////////////////
      }
      break;
    case PT_DYNAMIC:
      ...
      if ((reloc=(char *)malloc(phdrs[n].p_filesz))==0) {//не выдел€етс€ рама под динамический сегмент
#ifndef wintel
	fclose(fin, &iError);
#endif
	mfree(base);
	return -7;
      ...
      if (fread(reloc,phdrs[n].p_filesz,1,fin)!=1) {mfree(reloc); return -8;} //не читаетс€ динамический сегмент
#endif
      
      //ARM
#ifndef wintel
      if (fread(fin, reloc, phdrs[n].p_filesz, &iError)!=phdrs[n].p_filesz)
      {fclose(fin, &iError); mfree(reloc); mfree (base); return -8;}	//не читаетс€ динамический сегмент
#endif
      ////////////////////////////////////////////////////////////////////////
...
__arm void lnkloader_onload(WSHDR *filename, WSHDR *ext, void *param){
  char *fn=malloc(256);
  
  ws_2str(filename,fn,256);
  int size=get_file_size(fn);
  WSHDR *ws=AllocWS(size+1);
  char *buf=malloc(size+1);
  int f = fopen(fn,A_ReadOnly+A_TXT,P_READ,0);
  buf[fread(f,buf,size,0)]='\0';
...
{
  extern char swi_blib[64];
  extern unsigned int msg_conf;
  void *(*lt)[]=pLIB_TOP;
#define LIB_EMPTY ((void *)-1L)
  ...
  if ((sz=get_file_size(swi_blib))==-1) return;
  if (sz!=16384)
  {
    ShowMSG(1,(int)"Illegal library size!");
    return;
  }
  f=fopen(swi_blib,A_ReadOnly+A_BIN, P_READ, &ul);
  if (f==-1) return;
  lt=malloc(16384);
...
__no_init char smallicons_str[64];
__no_init char bigicons_str[64];

__no_init char lnksmallicons_str[64];
__no_init char lnkbigicons_str[64];
...
__arm void loading()
{
  extern unsigned int key_break;
  extern unsigned int png_load;
  extern unsigned int load_blib;
  
  static const int smallicons[2]={(int)smallicons_str,0};
  static const int bigicons[2]={(int)bigicons_str,0};
 
  
  static const int lnksmallicons[2]={(int)lnksmallicons_str,0};
  static const int lnkbigicons[2]={(int)lnkbigicons_str,0};
  ...
  static const REGEXPLEXT elf_reg=
  {
    ...
    bigicons,
#ifdef NEWSGOLD
    (int)"Open",    //LGP "ќткрыть"
    (int)"AltOpen", //LGP "ќпции"
    LGP_DOIT_PIC,
#endif
    ...
    static const REGEXPLEXT lnk_reg=
  {
    "lnk",
    0x54,//wtf?!
    0xFF,
    ...
    LGP_DOIT_PIC,
#endif
    (void *)lnkloader_onload,
    0
  };
  CreateHELPER_PROC();
  if(load_blib)LoadLibrary();
  if(png_load)InitPngBitMap();
  extern char folder_e[64];

  
  if(folder_e[strlen(folder_e)-1]!='\\')
{
  
  strcpy(smallicons_str,folder_e);
  strcpy(bigicons_str,folder_e);
  strcat(smallicons_str,"\\elf_small.png");
  strcat(bigicons_str,"\\elf_big.png");

  strcpy(lnksmallicons_str,folder_e);
  strcpy(lnkbigicons_str,folder_e);
  strcat(lnksmallicons_str,"\\lnk_small.png");
  strcat(lnkbigicons_str,"\\lnk_big.png");
...
#endif
  {
    extern unsigned int subproc;
    if(subproc)SUBPROC((void*)LoadDaemons);
    else LoadDaemons();
  }
...
#endif
{
  if (((unsigned int)s>>28)==0xA) //“олько строки из области RAM/FLASH
  {
    CutWSTR(ws,0); 
    ascii2ws(s,ws);
  }
...
__arm void NO_FUNC(int f)
{
  char s[32];
  extern unsigned int msg_missed;
  sprintf(s,"ќтсутствует функци€ %d(%03X)",f,f);
  if(msg_missed)ShowMSG(1,(int)s);
}
