...
void create_connect(RSS_FILE *frss)
{
  char rss_host[64];
...
int GetRSSEncoding(char *buf)
{
  int pos=0,lpos=0,msize=20;
  char *line=malloc(20);
  while(pos<strlen(buf))
  {
    ...
    {
      line[lpos]=0;
      if(strstr(line,"UTF"))return 0;
      if(strstr(line,"win"))return 1;
      return -1;
    }
    else 
    {
...
static int OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  if (msg->gbsmsg->msg==KEY_DOWN)
  {
    ...
    case GREEN_BUTTON:
      DNR_TRIES=3;
      SUBPROC((void *)create_connect, data->frss);
      SMART_REDRAW();
      break;
...
static int maincsm_onmessage(CSM_RAM *data, GBS_MSG *msg)
{
  ...
  if (msg->msg==MSG_HELPER_TRANSLATOR)
  {
    ...
    case ENIP_DNR_HOST_BY_NAME:
      ...
      {
	if (DNR_TRIES) SUBPROC((void *)create_connect,&csm->frss);
      }
      return(1);
    }
    if ((int)msg->data1==sock)
    {
      ...
      case ENIP_SOCK_CONNECTED:
        ...
        if (connect_state==1)
        {
          receive_mode=0;
          connect_state=2;
          SUBPROC((void *)send_req, &csm->frss);
        }
        break;
      ...
      case ENIP_SOCK_DATA_READ:
        ...
        if (connect_state>=2)
        {
          SUBPROC((void *)get_answer,&csm->frss);
        }
        break;
	...
	case -1:
	  connect_state=0;
          ...
          encoding=GetRSSEncoding(csm->frss.cur_xml);
