...
{
  Elf32_Ehdr ehdr;			// «аголовок эльфа
  Elf32_Phdr phdrs[MAX_PHNUM];	        // «аголовки програм
  Elf32_Word dyn[DT_BIND_NOW+1];	// “эги динамической секции
  ...
  if ((fin=fopen(filename, A_ReadOnly+A_BIN, P_READ, &iError))<0) return -1; // Ќе открываетс€ эльф
  if (fread(fin, &ehdr, sizeof(Elf32_Ehdr), &iError)!=sizeof(Elf32_Ehdr))    // Ќе читаетс€ эльф
  {fclose(fin, &iError); return -2;}
  
  
  if (*((long *)ehdr.e_ident)!=0x464C457F) // ƒа и не эльф это вовсе /* \x7FELF */
  {                               
    fclose(fin, &iError);
    return -3;
  }
  
  // ѕрочитаем все программные сегменты и вычислим необходимую область в RAM
  if (ehdr.e_phnum>MAX_PHNUM) return -9; // —лишком много програмных сегментов
  for(n=0;n<ehdr.e_phnum;n++)
  {
    if (lseek(fin, ehdr.e_phoff+n*ehdr.e_phentsize, S_SET, &iError, &iError2)!=ehdr.e_phoff+n*ehdr.e_phentsize)
    {fclose(fin, &iError); return -4;}				// Ќе найден программный заголовок
    if (fread(fin, &phdrs[n], sizeof(Elf32_Phdr), &iError)!=sizeof(Elf32_Phdr))
    {fclose(fin, &iError); return -5;}				// Ќе читаетс€ программный заголовок
  ...
  // ¬ыделим эту область и очистим ее
  if ((base=(char *)malloc(maxadr-minadr))==0) // Ќе выдел€етс€ пам€ть под эльф (Ќехватка свободной RAM)
  {
    fclose(fin, &iError);
    return -13;
  }
  
  // ќбнуление выделенной области
  zeromem_a(base,maxadr-minadr);
  
  // ќбход всех сегментов
  for(n=0;n<ehdr.e_phnum;n++)
  { 
    if (lseek(fin, phdrs[n].p_offset, S_SET, &iError, &iError)!=phdrs[n].p_offset)
    {fclose(fin, &iError); mfree(base); return -6;} // Ќе найден динамический сегмент

    switch (phdrs[n].p_type)
    {
     case PT_LOAD:
       
      // «агрузим программные сегменты с размером больше 0
      if (phdrs[n].p_filesz!=0) 
       {
	if (fread(fin, &base[phdrs[n].p_vaddr-minadr], phdrs[n].p_filesz, &iError)!= phdrs[n].p_filesz)
	{fclose(fin, &iError); mfree(base); return -10;} // Ќе читаетс€ программный сегмент
       }
        break;
     case PT_DYNAMIC:
       
      // ѕрочтем динамическую секцию
      if ((reloc=(char *)malloc(phdrs[n].p_filesz))==0) // Ќе выдел€етс€ пам€ть под динамический сегмент
      {
	fclose(fin, &iError);
	mfree(base);
	return -7;
      ...
      {fclose(fin, &iError); mfree(reloc); mfree (base); return -8;}	// Ќе читаетс€ динамический сегмент

      // ¬ытащим все тэги из динамической секции
      m=0;
	...
	while (m*sizeof(Elf32_Rel)<dyn[DT_RELSZ])
      ...
      default:	// Ќеизвестный тип програмного сегмента
       fclose(fin, &iError);
       mfree(base);
       return -11;
    }
