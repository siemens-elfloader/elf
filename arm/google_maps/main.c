...
if(mode==1)wsprintf(data->ws1,"%s %s:%d kb %s:%d",lgpData[LGP_LoadingMap],lgpData[LGP_Traffic],all_traf/1024,lgpData[LGP_State],connect_state);
else if(mode==0) wsprintf(data->ws1,"%s %s:%d kb %s:%d",lgpData[LGP_PressAnyKey],lgpData[LGP_Traffic],all_traf/1024,lgpData[LGP_State],connect_state);
else if(mode==2)wsprintf(data->ws1,"%s",lgpData[LGP_LoadingCache]);
  ...
  case KEY_UP: break;
    ...
    case KEY_DOWN:
       ...
       switch(msg->gbsmsg->submess)
         ...
         case GREEN_BUTTON:if(mode==0){Type=RIGHT;mapi=0;mapj=0;latitude=0;longitude=0;my_latitude=0;my_longitude=0;c();}break;
