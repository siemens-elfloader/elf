...
void log_pkt(char *txt)
{
  unsigned int io_error = 0;
                int h;
   
              h = fopen("4:\\1_pkt.txt",A_ReadWrite + A_Create + A_Append + A_BIN,P_READ+P_WRITE, &io_error);
            fwrite(h,txt,strlen(txt),&io_error);
            fwrite(h,"\n\r",2,&io_error);
            
            fclose(h,&io_error);
}

void log_process(char *txt)
{
  unsigned int io_error = 0;
                int h;
           
              h = fopen("4:\\1_process.txt",A_ReadWrite + A_Create + A_Append + A_BIN,P_READ+P_WRITE, &io_error);
            fwrite(h,txt,strlen(txt),&io_error);
...
//===================================================================
//Делаем SLI индикацию и счетчик сообщений в кл - Twitch
  ...
  while(j>0)
  {
        ...
        switch(RXbuf.pkt.type)
	{
	  ...
	  SMART_REDRAW();
          ...
          char s[255];
          sprintf(s,"T_LOGIN size: %d %s",n,RXbuf.data);
          log_pkt(s);
	  break;
	case T_XTEXT_ACK:
	case T_GROUPID:
	...
	case T_CLENTRY:
	  ...
	  GBS_SendMessage(MMI_CEPID,MSG_HELPER_TRANSLATOR,0,p,sock);
          ...
          if(RXbuf.pkt.type == T_XTEXT_ACK)
            sprintf(s,"T_XTEXT_ACK size: %d %s",n,RXbuf.data);
          if(RXbuf.pkt.type == T_GROUPID)
            sprintf(s,"T_GROUPID size: %d %s",n,RXbuf.data);
          
          if(RXbuf.pkt.type == T_GROUPFOLLOW)
            sprintf(s,"T_GROUPFOLLOW size: %d %s",n,RXbuf.data);
          
          if(RXbuf.pkt.type == T_CLENTRY)
            sprintf(s,"T_CLENTRY size: %d %s",n,RXbuf.data);
          
          if(RXbuf.pkt.type == T_CONTACTREMOVED)
            sprintf(s,"T_CONTACTREMOVED size: %d %s",n,RXbuf.data);
...
//	  snprintf(logmsg,255,LG_GRSTATUSCHNG,RXbuf.pkt.uin,*((unsigned short *)(RXbuf.data)));
	  GBS_SendMessage(MMI_CEPID,MSG_HELPER_TRANSLATOR,0,p,sock);
          ...
          {
          char s[255];
          if(RXbuf.pkt.type == T_STATUSCHANGE)
            sprintf(s,"T_STATUSCHG size: %d %s",n,RXbuf.data);
          
          if(RXbuf.pkt.type == T_CLIENTID)
            sprintf(s,"T_CLIENTID size: %d %s",n,RXbuf.data);
...
//          UpdateCSMname();
	...
	case T_CLIENT_ACK:
	  ...
	  GBS_SendMessage(MMI_CEPID,MSG_HELPER_TRANSLATOR,0,p,sock);
          {
          char s[255];
          sprintf(s,"T_CLIENT_ACK size: %d %s",sizeof(PKT)+2,RXbuf.data);
          log_pkt(s);
	  }break;
	case T_ECHORET:
	  {
	  ...
	  GBS_SendMessage(MMI_CEPID,MSG_HELPER_TRANSLATOR,0,p,sock);
          {
          char s[255];
          sprintf(s,"T_LASTPRIVACY size: %d %s",sizeof(PKT)+2,RXbuf.data);
          log_pkt(s);
	  }break;
	}
        ad(&RXbuf);
...
void ProcessPacket(TPKT *p)
{
  ...
  case T_STATUSCHANGE:
    ...
    if (t)
    {
      ...
      if (memcmp(t->xstate,&p->data[2],m))  // Если картинка икс статуса сменилась
      {
        zeromem(t->xstate,sizeof(t->xstate));
        memcpy(t->xstate,&p->data[2],m);
        FreeXText(t);
        if (t->xstate[0] && ENA_AUTO_XTXT)   // Если установлен икс статус и можно запрашивать по смене икс статуса :)
        {
          t->req_xtext=1;
