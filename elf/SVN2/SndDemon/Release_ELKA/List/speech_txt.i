...
inline void MPlayer_Toggle2()	{Send_MPlayer_Command(0x11,0);}
...
inline word PlayF3(const char *fpath,const char *fname,int vol ){
  ...
  if (!IsCalling())
  {
    ...
    i=PlayFile(0x10, sndPath, sndFName,0x4209 , 0x70BC, &_sfo1);    
...
void SpeechPhrases(char *str){
  ...
  tmp[i]=0;
  strcpy(gbs,(str+i)+1);  
  if ((tmp[0]!='#')&&(next=PlayF3("4" ":\\Zbin\\SndDemon",tmp,spc_vol))) {
    ;  
