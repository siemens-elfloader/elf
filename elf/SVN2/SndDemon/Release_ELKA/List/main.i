...
inline void MPlayer_Stop()	{Send_MPlayer_Command(0x0F,0);}	
inline void MPlayer_Start()	{Send_MPlayer_Command(0x0E,0);}	
inline void MPlayer_Next()	{Send_MPlayer_Command(0x02,0);}	
inline void MPlayer_Prev()	{Send_MPlayer_Command(0x01,0);}	
inline void MPlayer_Toggle()	{Send_MPlayer_Command(0x11,0);}
inline void MPlayer_Shutdown()	{Send_MPlayer_Command(0x0C,0);}	
inline void MPlayer_Mute()	{Send_MPlayer_Command(0x0A,0);}	
inline void MPlayer_Pause()	{Send_MPlayer_Command(0x10,0);}	
...
inline void ReadConfig(){
  ...
  if (spc_level)     llevel=(GetAkkuCapacity()-1)/5;  
...
extern const unsigned int spc_kukkey;
 ...
 void ProcessTime(byte cl){
  ...
  char tmp2[30];  
  tmp[0]=0;
  char ms[25]="v0aaavvvvvvvvvvvvvvvv0aa"; 
  sprintf (tmp2,"%st%d.amr;th%c.amr",cl?"thx.amr;":"",tm.hour,ms[tm.hour]);
  char ms2[11+11]="0ayyy00000000000000000";  
  if (tm.min<=20){         
    if (spc_min&&tm.min==0){
      sprintf (tmp,"%s;tx.amr;#",tmp2);      
    } else
    if (tm.min!=1&&tm.min!=2)
      sprintf (tmp,"%s;t%d.amr;tm%c.amr;#",tmp2,tm.min,ms2[tm.min]);
     else
       sprintf (tmp,"%s;t%de.amr;tm%c.amr;#",tmp2,tm.min,ms2[tm.min]);
  }else
   if (tm.min%10!=1&&tm.min%10!=2&&tm.min%10!=0)
     sprintf (tmp,"%s;t%d0.amr;t%d.amr;tm%c.amr;#",tmp2,tm.min/10,tm.min%10,ms2[tm.min%10]);
    else
     sprintf (tmp,"%s;t%d0.amr;t%de.amr;tm%c.amr;#",tmp2,tm.min/10,tm.min%10,ms2[tm.min%10]);
...
void ProcessAccum(void){
  char tmp[64];
  byte lvl= GetAkkuCapacity();
  byte lvl10=(lvl-1)/5;
  byte LS=*RamLS();
  if ((zds)==0&&LS)  {    
    SpeechPhrasesChk("found.amr;#");    
    llevel=lvl10;
  }else
  if ((zds)!=3&&(LS==3))      SpeechPhrasesChk("full.wav;#");
   else
  if ((zds)!=0&&(LS==0))      SpeechPhrasesChk("found0.amr;#");
  zds=LS;

  if (LS!=3&&((lvl10/2)!=(llevel/2))){
    tmp[0]=0;
    if (LS)
      sprintf (tmp,"up.amr;%d0.amr;percent.amr;#",lvl10/2);
    else
     if (lvl<spc_level) sprintf (tmp,"down.amr;%d0.amr;percent.amr;#",(lvl10/2)+1);
    if (tmp[0]!=0)      SpeechPhrasesChk(tmp);     
  };
  if (lvl<6&&llevel==1&&LS!=3) SpeechPhrasesChk("down.amr;5.amr;percent.amr;#"); 
  llevel=lvl10;
};
...
void ProcessNet(){
  ...
  if (rds!=rdsc){
    if (rds) SpeechPhrasesChk("sp.amr;#"); 
     else SpeechPhrasesChk("sn.amr;#"); 
    rds=rdsc;
  }
}
...
int my_keyhook(int submsg, int msg)
{
  ...
  if (msg==0x194){
    if (!IsCalling()){  
       ...
       if (submsg!=0x0C&&submsg!=key_prev&&submsg!=key_next&&submsg!=0x27&&submsg!=35&&((!IsUnlocked()&&spc_kukloc)||(submsg==spc_kukkey&&vl))){        
         if (klong){
           ProcessTime(0);
           klong=0;      
           return 2;    
       ...
       if (spc_keyvol&&IsUnlocked()&&TopG(0x4C)&&!st){
...
int MyIDLECSM_onMessage(CSM_RAM* data,GBS_MSG* msg)
{
  ...
  if (msg->msg==0x15680){
    ...
    if ((   ((char *)(((int *)msg->data1)[6]))[0x11]  )==0){
      ...
      if (loc&&(!batt_level||(GetAkkuCapacity()>batt_level))){
...
int MainCSM_onMessage(CSM_RAM* data,GBS_MSG* msg)
{
  ...
  if (msg->msg== 0xDF01){
    ShowTag();
    GBS_StartTimer(&mytmr2,refresh, 0xDF01,0,0x4209);
  ...
  if (msg->msg==0x70BC)  {
    if ((msg->submess>>16)==next){    
      word kd=(msg->submess);
      if (kd==7||kd==5)        SpeechPhrases(gbs);else;        
    };
...
static void maincsm_oncreate(CSM_RAM *data)
{
  ...
  if (ALLOW_HELLO_MSG) ShowMSG(1,(int)("SndDemon v1.50" " loaded."));  
  sndPath=AllocWS(256);
  sndFName=AllocWS(256);

  AddKeybMsgHook((void *)my_keyhook);   
  
  GBS_StartTimerProc(&mytmr,216*3,TimerProc);  

  GBS_SendMessage(0x4209, 0xDF01);
...
{
  wsprintf((WSHDR *)(&MAINCSM.maincsm_name),"SndDemon v1.50");
}
