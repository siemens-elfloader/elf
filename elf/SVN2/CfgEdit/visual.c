...
extern void ClearT9Key(char *T9_key);
extern void AddT9Key(char *T9_key,int chr);
extern void BackSpaceT9(char *T9_key);
extern const char table_T9Key[];

int flist_id;
int flist_prev_itemcount;
char flist_hdr_text[128];
char flist_sk_r[16];
char flist_header[128];
char FListT9Key[32];
...
FLIST *FindFLISTtByNS(int *i, int si)
{
  ...
  {
    s=FListT9Key;
    if(si==IS_FOLDER) d=fl->name+1;// пропустим слеш \ в имени папки
    else d=fl->name;
...
void UpdateFListHeader(void)
{
  ...
  {
    strcpy(flist_hdr_text,"Input T9:");
    strcat(flist_hdr_text,FListT9Key);
    strcpy(flist_sk_r,"<C");
  }
  else
  {
    strcpy(flist_hdr_text,flist_header);
    strcpy(flist_sk_r,"Close");
  }
}
...
void RecountFList()
{
  ...
  UpdateFListHeader();
  if (!flist_id) return; 
  data=FindGUIbyId(flist_id,NULL);

  i=CountFList();
  if (i!=flist_prev_itemcount)
  {
    flist_prev_itemcount=i;
    Menu_SetItemCountDyn(data,i);
  }
  SetCursorToMenuItem(data,0);
  if (IsGuiOnTop(flist_id)) RefreshGUI();
}
...
void open_select_file_gui(void *ed_gui, int type)
{
  ...
  UpdateFListHeader();
  flist_prev_itemcount=n;
  patch_header(&filelist_HDR);
  flist_id=CreateMenu(0,0,&filelist_STRUCT,&filelist_HDR,0,n,fview,0);
}
