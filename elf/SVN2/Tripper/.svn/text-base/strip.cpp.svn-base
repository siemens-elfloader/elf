...
(c)–азработчики эльфпака(Rst7/CBSIE, BoBa и другие), Captain_SISka
*/
...
Elf32_Phdr p[MAX_PHNUM];	                                        //заголовки програм
int np;
char * pd[MAX_PHNUM];
int oldsz,newsz;
//--------------------------------------------------
...
//				if (fread((void *)&base[phdrs[n].p_vaddr-minadr],phdrs[n].p_filesz,1,fin)!=1) return -11;	//не читаетс€ програмный сегмент
            ...
            if (fread((void *)pd[np],1,phdrs[n].p_filesz,fin)!=phdrs[n].p_filesz) return -100-n;	//не читаетс€ програмный сегмент
            np++;
		      }
		      break;
...
if(opt_info)		      cout << "dyn seg: off="<<hex<<phdrs[n].p_offset<<", sz="<<phdrs[n].p_filesz<<endl;
...
FILE * f;
if ((f=fopen(filename,"wb"))==NULL) return -1;			//не открываетс€ ельф
//calc phtable offset
int ts=0;
