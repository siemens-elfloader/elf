...
void SLI_On()
{
  ...
  {
    SLI_SetState(1);   
    GBS_StartTimerProc(&sli_tmr,30, SLI_Off);
  }
}
...
void get_answer(void)
{
  ...
  while(j>0)
  {
	...
	case 6:
          ...
          {
            snprintf(logmsg,255,"ERR: %s","Íîìåðîì çàøëè ñ äðóãîãî êëèåíòà");
            disautorecconect = 1;
          }
          else
          {
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  if (msg->msg==0xDEAE)
  {
    ...
    case 0x80:
      ...
      snprintf(logmsg, 255, "GPRS ñåññèÿ çàïóùåíà, ïîäêëþ÷åíèå ÷åðåç %d ñåêóíä...", RECONNECT_TIME);
      GBS_StartTimerProc(&reconnect_tmr,216*RECONNECT_TIME,do_reconnect);
      return(1);
    case 0x94:
	...
	if (!disautorecconect)
        {
          GBS_StartTimerProc(&reconnect_tmr,216*RECONNECT_TIME,do_reconnect);
          snprintf(logmsg,255,"%s\nÎøèáêà, ïåðåïîäêëþ÷åíèå ÷åðåç %d ñåêóíä...",logmsg, RECONNECT_TIME);
        }
	break;
      }
    }
