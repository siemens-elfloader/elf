...
void parse_name(char* fname, ParseFileNameStruct* pfns)
{
	...
	char* ps = GetFileExt(pfns->cleanName) - 1; // Указатель на точку перед расширением
	if (ps - pfns->cleanName > 3) // Кроме точки должно быть как минимум 3 символа - скобки и число
	{
...
int find_next_name(ParseFileNameStruct* pfns)
{
	...
	{
		sprintf(buf, _s_i, pfns->cleanName, pfns->number);
		if (!fexists(buf))
			return 1; 
	}
	return 0;
...
int fscp(char *src, char *dst, int ip)
{
	...
	else
	{
		if (isSame)
		{
			...
			ParseFileNameStruct pfns;
			parse_name(src, &pfns);
			// Ищем следующее незанятое имя
			if (find_next_name(&pfns))
			{
				char buf[MAX_PATH];
				sprintf(buf, _s_i, pfns.cleanName, pfns.number);
				res = fcopy(src, buf);
