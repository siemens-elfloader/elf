...
extern const char text_bt_off[128];
extern const char text_bt_on[128];
extern const unsigned int img_x;
extern const unsigned int img_y;
extern const char img_bt_off[128];
extern const char img_bt_on[128];
...
void InfoBT()
{
  ...
  if(ena_img)
  {
  DrawCanvas(canvasdata,img_x,img_y,img_x+GetImgWidth((int)img_bt_on)-1,img_y+GetImgHeight((int)img_bt_on)-1,1);
  DrawImg(img_x,img_y,(int)img_bt_on);
  }
  //Текст
  if(ena_text)
  {
  ...
  if(ena_img)
  {
  DrawCanvas(canvasdata,img_x,img_y,img_x+GetImgWidth((int)img_bt_off)-1,img_y+GetImgHeight((int)img_bt_off)-1,1);
  DrawImg(img_x,img_y,(int)img_bt_off);
  }
  //Текст
  if(ena_text)
  {
  ...
  {
  case 0:
  wsprintf(ws_txt,"%s",text_bt_off);
  DrawString(ws_txt,text_x,text_y,screenw,screenh,font_txt,TEXT_OUTLINE,color,frame_color);
  break;
  case 1:
  sprintf(xz,"%s",text_bt_off);
  PrintField(text_x,text_y,xz,align,font,space);
  break;
  }
  }
...
int maincsm_onmessage(CSM_RAM* data,GBS_MSG* msg)
{
  ...
  if(ena_req) ShowMSG(1,(int)"Конфиг ActivityBT обновлен!");
  FontPathFree();
  InitConfig();
...
{
  wsprintf((WSHDR *)(&MAINCSM.maincsm_name),"ActivityBT - (c) KIBER-DEMON");
}
...
int main()
{
  ...
  if(ena_msg) ShowMSG(1,(int)"ActivityBT установлен!");
  return 0;
}
