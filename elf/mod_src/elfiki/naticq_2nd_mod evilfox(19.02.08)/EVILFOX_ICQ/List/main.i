...
GBSTMR ctmr; 
char online_txt[18]; 
extern const unsigned int ONTIME_X; 
extern const unsigned int ONTIME_Y; 
extern const int Pause_Time; 
int align=0; 
int fonto=1; 
int space=1; 
char fontpath[]="2:\\font\\"; 
int time_draw; 
void *canvasdata; 
int show_ping; 
int ping_sec; 
extern const int Show_Ping; 
...
int pp_color; 
extern const int Is_DRAWMAIN_On; 
int main_d; 
extern const unsigned int MSG_Y; 
extern const unsigned int MSG_X; 
int paused_height; 
int in_count; 
int out_count; 
char last_msg_buffer[3072]; 
int comanda; 
int msg_away_timer; 
...
extern const char away_msg1[24]; 
...
char mod_info[]="2nd generation mod based on revision 1848! mod date: 19.02.08 14:02!"; 
...
void SLI_Off()
{
  SLI_SetState(2);
  if (total_unread)
    GBS_StartTimerProc(&mytmr, 21, SLI_On); 
  else
    SLI_SetState(0);
}
...
{
  if(Is_SLI_On)
  GBS_StartTimerProc(&mytmr, 262, SLI_On); 
}
...
void Init_Ext_Sounds(void)
{
  ...
  while(t=(CLIST *)(t->next))
  {
    if(!t->isgroup)
    {
      ...
      hFile_ex = fopen(ext_sound,0 + 0x8000,0x80, &io_error_ex);
      if(hFile_ex!=-1)
      {
        in_ext++;
        t->ext_snd=1;
...
void Save_Contacts_file(void)
{
  ...
  while(t=(CLIST *)(t->next))
  {
    if((!t->isgroup)&&(!t->ext_snd))
    {
      ...
      hFile_cl = fopen("4:\\CL.txt",2 + 0x100 + 8 + 0x8000,0x80+0x100, &io_error_cl);
      if(hFile_cl!=-1)
      {
...
static const char def_clm_hdr_text[] = "C-List..";
static const char key_clm_hdr_text[] = "T9 Key: ";
...
char clmenu_sk_r[16];
static const char def_clmenu_sk_r[] = "Close";
...
ProcessPacket(TPKT *p)
{
  ...
  case 17:
    ...
    if (q&&(p->pkt.type==16)&&Is_ILU_On) IlluminationOn(ILL_DISP_SEND,ILL_KEYS_SEND,ILL_SEND_TMR,ILL_RECV_FADE); 
...
void method0(MAIN_GUI *data)
{
  ...
  DrawString(data->ws2,3,scr_h-8-GetFontYSIZE(5)-GetFontYSIZE(5),
...
int maincsm_onmessage(CSM_RAM *data,GBS_MSG *msg)
{
  ...
  MAIN_CSM *csm=(MAIN_CSM*)data;
  {
	...
	DrawImg(IDLEICON_X,IDLEICON_Y,S_ICONS[icn]);
	  ...
	  DrawImg(IDLEICON_X,IDLEICON_Y,XStatusesIconArray[CurrentXStatus]);
          ...
          if(Is_Online_On)
          PrintField(ONTIME_X,ONTIME_Y, online_txt, align, fonto, space);
          if(Show_Ping)
            PrintField(PING_X,PING_Y, ping_txt, align, fonto, space);
          if(Is_Unread_On)
          {
            sprintf(unread_txt, "Unread: %d", total_unread2);
            PrintField(UNREAD_X,UNREAD_Y, unread_txt, align, fonto, space);
          }
  ...
  if (msg->msg==0xDEAE)
  {
	...
	if (connect_state==1)
	{
	  ...
	  GROUP_CACHE=0;
	  SENDMSGCOUNT=0; 
	  if (!FindGroupByID(0)) AddGroup(0,"Not in list");
...
{
  static const SOFTKEY_DESC sk={0x0FFF,0x0000,(int)"Menu"};
  static const SOFTKEY_DESC sk_cancel={0x0FF0,0x0000,(int)"Close"};
...
void SendAuthGrant(GUI *data)
{
  ...
  CLIST *t;
  int l;
  const char s[]= "You are authorized!";
  if ((t=ed_struct->ed_contact)&&(connect_state==3))
  {
    p=malloc(sizeof(PKT)+(l=strlen(s))+1);
    p->pkt.uin=t->uin;
...
{
  {0,(int)"Quote",    0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Add smile",   0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Get short info",   0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Add/rename",   0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Send Auth Req", 0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Send Auth Grant", 0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Open logfile",  0x7FFFFFFF,0,0,3,0x5A2},
  {0,(int)"Clear log", 0x7FFFFFFF,0,0,3,0x5A2}
};
