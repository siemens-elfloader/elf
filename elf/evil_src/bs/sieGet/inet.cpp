...
void Download::onHTTPData(char * data, int size) // Запись данных в файл
{
  ...
  if(ballet_mode) SendToBallet(file_name, file_size, file_loaded_size, 1, NULL);
...
void Download::onHTTPStopped()
{
  ...
  hFile = -1;
  if(ballet_mode) SendToBallet(NULL, NULL, NULL, 2, NULL);
...
void Download::onHTTPFinish()
{
  ...
  if(ballet_mode) SendToBallet(NULL, NULL, NULL, 0, full_file_name);
