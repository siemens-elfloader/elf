...
//      main_csm->sel_bmk=CreateBookmarksMenu();
      char *fname;
      fname = getSymbolicPath("$bookmarks\\");
...
char TIMEZ[16];
int sieget_o;
char sieget_file[128];

void RunFILE()
{
  WSHDR *elfname=AllocWS(256);
  wsprintf(elfname,sieget_file);
...
void GetTime()
{
    ...
    sprintf(TIMEZ, "%02d:%02d", time.hour, time.min); 
}

void loads()
{
  timer++; if(timer==6) timer=1;
      sprintf(menu_img2, "%s\\w%d.png", menu_img, timer);
#ifdef ELKA
...
#endif
#endif
      GBS_StartTimerProc(&loadingo, 20, loads);
}

extern const char colorz[4];

//=============================sieget============================
char sieget_data[128];
int sieget_d;
void ShowDownload()
{
  WSHDR *ws=AllocWS(128);
  wsprintf(ws, sieget_data);
  sprintf(menu_img2, "%s\\lower_menu.png", menu_img);
  DrawImg(0,0,(int)menu_img2);
#ifdef ELKA
  DrawImg(2,2,(int)"4:\\zbin\\sieget\\img\\downloading.png");
  DrawString(ws,20,0,240,20,FONT_SMALL,0,colorz,GetPaletteAdrByColorIndex(23)); 
#else
  sprintf(menu_img2, "%s\\sieget_d.png", menu_img);
  DrawImg(0,0,(int)menu_img2);
  DrawString(ws,15,1,132,13,FONT_SMALL,0,colorz,GetPaletteAdrByColorIndex(23)); 
#endif
  FreeWS(ws);
}
//=============================sieget============================
...
static void method0(VIEW_GUI *data)
{
  ...
  if (data->gui.state==2)
  {
    ...
    if(!STOPPED)
    {
    sprintf(menu_img2, "%s\\background.png", menu_img);
    DrawImg(0,0,(int)menu_img2);
    }
//    DrawImg(0,0,(int)"4:\\test.png");
    RenderPage(vd,1);
...
/*      DrawRectangle(0,h1,w1,scr_h,0,
      ...
      {
        DrawRectangle(1,h1+3,vd->loaded_sz*(w1-1)/vd->page_sz,scr_h-1,0,
      ...
      DrawString(data->ws1,0,h1+2,w1,scr_h,FONT_SMALL,TEXT_ALIGNMIDDLE,
        colorz,GetPaletteAdrByColorIndex(23));   
...
/*#ifdef ELKA
      ...
      DrawImg(0,300,(int)menu_img2);
      GetTime();
      wsprintf(data->ws2,"bat:%d%  Time:%s", *RamCap(), TIMEZ);
      int h1;
      h1=scr_h-GetFontYSIZE(FONT_SMALL)-2;
//      w1=scr_w-Get_WS_width(data->ws2,FONT_SMALL)-2;
      DrawString(data->ws2,0,h1+3,240,scr_h+1,FONT_SMALL,TEXT_ALIGNMIDDLE,colorz,GetPaletteAdrByColorIndex(23)); 
#else
      sprintf(menu_img2, "%s\\lower_menu.png", menu_img);
#ifdef C65
      DrawImg(0,117,(int)menu_img2);
      ...
      h1=scr_h-GetFontYSIZE(FONT_SMALL)-2;
//      w1=scr_w-Get_WS_width(data->ws2,FONT_SMALL)-2;
      DrawString(data->ws2,0,h1+3,132,scr_h+1,FONT_SMALL,TEXT_ALIGNMIDDLE,colorz,GetPaletteAdrByColorIndex(23)); 
#endif*/
    }
    if(sieget_d) ShowDownload();
//    DrawClock();
  }
}
...
void check_url(char *link)
{
  linkz=0;
  int len=strlen(link);
  if((strstr(link, ".jpg")&&(link[len-1]=='g'))) linkz=1;
  if((strstr(link, ".zip")&&(link[len-1]=='p'))) linkz=1;
  if((strstr(link, ".jar")&&(link[len-1]=='r'))) linkz=1;
  if((strstr(link, ".png")&&(link[len-1]=='g'))) linkz=1;
  if((strstr(link, ".rar")&&(link[len-1]=='r'))) linkz=1;
  if((strstr(link, ".jad")&&(link[len-1]=='d'))) linkz=1;
  if((strstr(link, ".bmp")&&(link[len-1]=='p'))) linkz=1;
  if((strstr(link, ".7z")&&(link[len-1]=='z'))) linkz=1;
}
...
static int method5(VIEW_GUI *data,GUI_MSG *msg)
{
  ...
  if((m==KEY_DOWN)||(m==LONG_PRESS))
  {
    ...
    {
    case ENTER_BUTTON:
      if(m==LONG_PRESS) break;
      if (vd->pos_cur_ref==0xFFFFFFFF) break;
      //================check sieget
      if ((rf = FindReference(vd,vd->pos_cur_ref))&&(!sieget_d)&&(!sieget_o))
        {
          if (rf->id != _NOREF)
          {
    ...
    case GREEN_BUTTON:
      ...
      if(sieget_o)
      {
        ...
        sipc->name_from=ipc_my_name;
        sipc->data=NULL;
        GBS_SendMessage(MMI_CEPID,MSG_IPC,IPC_BREAKD,sipc);
      }
      break;
    case 0x38:
      if(sieget_o)
  ...
  if (msg->msg==MSG_IPC)
  {
    ...
    if ((ipc=(IPC_REQ*)msg->data0))
    {
     ...
     //       int l=strlen(ipc->data);
            strcpy(sieget_data, ipc->data);
            sieget_d=1;
            DirectRedrawGUI();
          break;
        case IPC_TOBALLET2:
          sieget_o=1;
          strcpy(sieget_file, ipc->data);
          strcpy(sieget_data, "Done!! 7-Open, 8-Close");
          DirectRedrawGUI();
          break;
        case IPC_TOBALLET3:
          sieget_o=0;
