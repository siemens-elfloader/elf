...
void play_third()
{
  ...
  GBS_StartTimerProc(&mytmr2, 22*lenght+30,play_forth);
}
...
void play_second()
{
  ...
  if(strlen(zime)!=3) GBS_StartTimerProc(&mytmr2, 22*lenght+30,play_third);
  else play_third();
}
...
void get_talk()
{
  ...
  if((ENA_TALK)&&(minimum<time.hour)&&(maximum>time.hour))
  {
    if(talk_len==2)
    {
      ...
      GBS_StartTimerProc(&mytmr2, 22*lenght+30,play_second);
      }
    }
    else
    {
