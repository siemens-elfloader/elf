...
//DrawCanvas(data, 0, 0, 239, 319, 1);
   ...
   DrwPngImg(FImage4, 0,24);//фон
   DrwPngImg(FImage10, 4,3);//иконбар
  //========================================================  
...
WSHDR * ws = AllocWS( 124 );
  ...
  {        
    ...
    if (bond)
    {
       ...
       {
         GBS_DelTimer(&idle_timer);
         GBS_StartTimerProc(&idle_timer, 2, Mod4);
       }
       else
       {
         GBS_DelTimer(&idle_timer);
         GBS_StartTimerProc(&idle_timer, TMR_SECOND*4, Mod4);
       }
    }
  }
  UnlockSched();
...
int OnKey(MAIN_GUI *data, GUI_MSG *msg)
{
  ...
  if ((msg->gbsmsg->msg==LONG_PRESS))// Если клавиша зажата
  {
    if (bond && state>3) 
    {
      ...
      GBS_StartTimerProc(&sleep_timer, TMR_SECOND*5, TimeSleep);
    }
