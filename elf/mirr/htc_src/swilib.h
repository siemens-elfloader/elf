...
typedef struct
{
  ...
  char w;
  char h;
  unsigned short bpnum; //For BW=1, 8bit=5, 16bit=8, 32bit=10,  0x80 - packed,
#endif
  char *bitmap;
}IMGHDR;
...
#pragma swi_number=0x8294
__swi __arm char *RamAlarmclockHour();

#pragma swi_number=0x8295
__swi __arm char *RamAlarmclockMinute();



//0A80: 0xA05C56F8 ; 2A0: int GetAlarmclockState(int state);
#pragma swi_number=672
__swi __arm int GetAlarmclockState(int state);
...
__swi __arm char *RamRingtoneStatus(void);
...
//------------------------------------------------------------------------------------------------------------
#ifndef DOS
externC char *strcpy(char *to, const char *from);
externC char *strcat(char *to, const char *from);
externC unsigned int strlen(const char *);
externC int strcmp(const char *,const char *);
//externC int stricmp(const char *,const char *);
externC int strncmp(const char *,const char *, int size);
//externC int strnicmp(const char *,const char *, int size);
//externC int memcmp(void *s1, void *s2, int size);
externC void *memcpy(void *to, const void *from, int size);
//extern void *memcopy(void *to, const void *from, int size);
externC void *memmov(void *to, void *from, int size);
externC void *movsd(void *to, void *from, int size);//быстрое копир-е 4б блоками
externC void *movsw(void *to, void *from, int size);//копир-е 2б блоками
//void *memmove(void *to, void *from, int size);//копир-е пересек-ся блоков
externC void *memset(void *mem, int val, int size);
externC void *memsetw(void *mem, int val, int size);
externC void *memsetd(void *mem, int val, int size);

//inline void  zeromem (void *dest,int n){  memset(dest,0,n); }
#endif
//------------------------------------------------------------------------------------------------------------
externC int iszero(void *mem, int dsize);
...
externC char * strrchr (const char *s,int c);
