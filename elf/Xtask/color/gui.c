  ...
  do
  {
    if (icsm==ircsm)
    {
	 ...
	 /* WSHDR *ws1=AllocWS(128); 
	  ...
	  wsprintf(ws1, "0:%d Kb",f0);
	  DrawString(ws,2,15,15,15,7,0,GetPaletteAdrByColorIndex(4),GetPaletteAdrByColorIndex(23)); 
      */
      WSHDR *ws = AllocWS(256);
      int f0 = GetFreeFlexSpace(0, ErrorNumber)/1024;
      wsprintf(ws,"%d kb | %d kb",GetFreeRamAvail()/1000,f0);
...
//s=(time.min<10)?":0":":";
sprintf(mmenu_hdr_txt,"%s %s%d %s | %d %% | %d db",days[dayy],s1,date.day,mounth[date.month],*RamCap(),rn->power);
...
void bm_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  if (s)
  {
    if (strlen(s))
    {
      ...
      wsInsertChar(ws,0x0004,1);//Цвет надписей
      wsInsertChar(ws,0xE008,1);//
...
void ShowBMmenu(void)
{
  ...
  CreateMultiLinesMenu(0,0,&bm_menu,&bm_menuhdr,0,9);
