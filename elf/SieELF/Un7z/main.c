...
SizeT outProcessed=0; //Скольк?распаковалос?//char newname[128];
char filename[256];
unsigned int err;  
...
unsigned char *un7z(int inFile, UInt32 compressedSize, UInt32 outSize){ //* на буфе??распак. данным?//  int res;                                            //outProcessed = скольк?распаковалос?  
  SizeT inProcessed;
...
do{
  ...
  *file++='\\';
  ...
  if (b[0]==1 && b[1]==4 && b[2]==6){ //Распакованны?заголово?!
...
//  fclose(outFile);
	///////////
	if (b[0]!=1 || b[1]!=4 || b[2]!=6 || b[3]!=0) //Распакованны?заголово?!
		break;
	getpos=4;
	maxcnt=get8b(b);
	if(b[getpos++]!=9) break;
	...
	get8b(b);
	empty=0;
	if(b[getpos]==0xe){	//Пустые заголовк?	    ++getpos;
