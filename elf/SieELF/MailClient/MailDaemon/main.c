  ...
  case POP_DELETE_MESSAGE:
    if (pop_stat.pop_state==POP_RECEIVE_HEADERS||pop_stat.pop_state==POP_RECEIVE_MESSAGE)
    {
      if (!mes_rec)
      {
        ...
        {
          end_connect(pop_stat.pop_state==POP_RECEIVE_HEADERS?"不能写入标题!":"不能写入信息!");
          return;
        }
        mes_rec=1;
        return;
  ...
  *socklasterr()=0;
  ...
  if (p_res)
  {
	...
	else
	{
      ...
      {
	LockSched();
	ShowMSG(1,(int)"不能创建socket,请重新启动GPRS!");
	UnlockSched();
...
int maincsm_onmessage(CSM_RAM* data,GBS_MSG* msg)
{
  ...
  if (msg->msg==MSG_HELPER_TRANSLATOR)
  {
	...
	{
	  ShowMSG(1,(int)"不合法的邮件 ENIP_SOCK_CONNECTED!");
	}
	break;
	...
	{
	  ShowMSG(1,(int)"不合法的邮件 ENIP_DATA_READ");
	}
	break;
      ...
      case ENIP_BUFFER_FREE:
        if (!sendq_p)
        {
          ShowMSG(1,(int)"不合法 ENIP_BUFFER_FREE!");
