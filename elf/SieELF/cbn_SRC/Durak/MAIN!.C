...
inline void showcurrent(){
  char *s=current, *s1=tmp1, *s2=tmp2, d=(strlen(current)&1)==0;
  while(*s){
    if(!d) 
      *s1++=*s++;
    else d=0;
...
int findmin(char card){ //найти мин карту данной масти -1=нет
  ...
  forg{
    if((girl[i]>>4)==(card>>4) && girl[i]>card)
      if(pos<0 || girl[i]<girl[pos]) pos=i;
  }
  return pos;//i error!
}
...
lget:      pickall(girl);  pickup(); return; 
    ...
    if((i=findmin(cur))<0){ //нет козыря побить
      goto lget;
    }
  }else{ //не козырь
...
void AIcont(){ //атакуем продолжаем // найти минимальную из карт (продолжить атаку)
  int j,i,k=-1,l,m=1000;
  for(j=0;j<strlen(current);j++)
    forg{
      if((girl[i]>>4)!=kozir && (l=(girl[i]&0xf))==(current[j]&0xf)) if(l<m){ m=l; k=i;}
    }
  if(k<0){
    *current=0; //отбой
    pickup();
...
void AIatack(){ //атакуем с нуля (ищем пары с чего походить)
  int i,k,flag=0;
lll:  
  ...
  if(k>=6){ //нет пар - берем минимальную некозырную карту
    ...
    if(k>=9){ //все козыри - ходим с минимального
      if(flag){/////error randcard here!!!
         if((i=findmin((kozir<<4)+5))<0) i=0; // от балды
         goto allll;
      }else{ 
        flag=1;
        goto lll;
...
//  xod=0;//rand()&1;
  ...
  *(char*)(girlbmp+5)=level+'0';
  ...
  if(fon) mfree(fon);
lll:  
  if(fon=loadgraph(filename)){
    if(*(short*)fon==width && *(short*)(fon+2)==height) fastfon=1;
  }else 
    if(level){ level=0; goto lll;}
  //
...
void ai(){ //искусственный интеллект
  if(!*current) AIatack();
  else if(strlen(current)&1) AIdefence(); 
  else AIcont(); 
//  xod=1;
  REDRAW();
  if(!*coloda){
    if(!strlen(player)){ ++level; newgame();}   //levelup
    else if(!strlen(girl)){ if(level>0) --level; newgame();}
    //НЕВЕРНО!!! нужны еще проверки.. добавление колоды и т.п.
  }
}
