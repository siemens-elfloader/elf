...
int cpu3_emulate(int cycles) //Õ≈Œœ“»Ã»«»Ó‚‡ÌÌ‡ˇ ˝ÏÛÎˇˆËˇ (ÒÓ‚ÏÂÒÚËÏ‡) !!!!!
{
//  debug_trace=1;
  int i;
	byte op, cbop;
...
#pragma optimize=no_code_motion
void cpu_emulate(int cycles) // Œœ“»Ã»«»–Œ¬¿ÕÕ¿ﬂ ˝ÏÛÎˇˆËˇ  (cpu3_emulate = ÌÂÓÔÚËÏËÁ)
{
	byte op, cbop;
	int clen;
	static union reg acc;
...
do{
  /*
  if (!(cpu.halt && IME)) goto aaa;
  if (R_IF & R_IE){ cpu.halt = 0; goto aaa;}
  if ((R_IE & (IF_VBLANK | IF_STAT)) && (cycles > cpu.lcdc))
	cycles = cpu.lcdc;
  if (!((R_IE & IF_TIMER) && (R_TAC & 0x04)))
     cpu_timers(clen=cycles);
  else
*/
//  if (!(clen = cpu_idle(cycles)))
	{
//       if (IME && (IF & IE)){
//		PRE_INT;
		switch ((IF & IE))
		...
		case 0x11: case 0x13: case 0x15: case 0x17:
		case 0x19: case 0x1B: case 0x1D: case 0x1F:
			PRE_INT;THROW_INT(0); break;
		case 0x02: case 0x06: case 0x0A: case 0x0E:
		case 0x12: case 0x16: case 0x1A: case 0x1E:
			PRE_INT;THROW_INT(1); break;
		case 0x04: case 0x0C: case 0x14: case 0x1C:
			PRE_INT;THROW_INT(2); break;
		case 0x08: case 0x18:
			PRE_INT;THROW_INT(3); break;
		case 0x10:
			PRE_INT;THROW_INT(4); break;
		}
//	}
//	IME = IMA;

	clen = cycles_table[op=FETCH];
...
//			op, (PC-1) & 0xffff, mbc.rombank);
	...
	//div_advance(clen);//Ì‡‰Ó "›“Œ ¬—≈ Õ¿ƒŒ!!!!!!!!!
  /*
  if((cpu.div += (clen<<1))>= 256)
	{
		R_DIV += (cpu.div >> 8);
		cpu.div &= 0xff;
	}

	//timer_advance(clen);//Ì‡‰Ó
 	int unit, tima;
...
*/
//	lcdc_advance(clen);////////˝ÚÓ:
  	if((cpu.lcdc -= (clen=clen<<1>>cpu.speed))<=0)
	    lcdc_trans();
}	
}while((cycles-=clen) > 0);// goto next;
}
