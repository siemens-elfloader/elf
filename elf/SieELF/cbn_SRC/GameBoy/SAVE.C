...
void loadstate(int f)
{
	...
	lseek(f, iramblock<<12, 0, &err, &err);
	fread(f, ram.ibank, 4096 * irl, &err);
	
	lseek(f, vramblock<<12, 0, &err, &err);
	fread(f, lcd.vbank, 4096 * vrl, &err);
	
	lseek(f, sramblock<<12, 0, &err, &err);
	fread(f, ram.sbank, 4096 * srl, &err);
}
...
void savestate(int f)
{
	...
	lseek(f, 0, 0, &err, &err);
	fwrite(f, buf, 4096, &err);
	
	lseek(f, iramblock<<12, 0, &err, &err);
	fwrite(f, ram.ibank, 4096 * irl, &err);
	
	lseek(f, vramblock<<12, 0, &err, &err);
	fwrite(f, lcd.vbank, 4096 * vrl, &err);
	
	lseek(f, sramblock<<12, 0, &err, &err);
	fwrite(f, ram.sbank, 4096 * srl, &err);
}
