...
void eepromSaveGame(gzFile gzFile1)
{
  utilWriteData(gzFile1, eepromSaveData);
  utilWriteInt(gzFile1, eepromSize);
  utilGzWrite(gzFile1, eepromData, 0x2000);
}

void eepromReadGame(gzFile gzFile1, int version)
{
  utilReadData(gzFile1, eepromSaveData);
  if(version >= SAVE_GAME_VERSION_3) {
    eepromSize = utilReadInt(gzFile1);
    utilGzRead(gzFile1, eepromData, 0x2000);
  } else {
...
void eepromWrite(u32 aa/* address */, u8 value)
{
  if(cpuDmaCount == 0)
    return;
  int bit = value & 1;
