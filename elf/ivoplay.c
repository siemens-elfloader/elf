...
char ttt[128]="";
int random=0,kskins=0,mp3length=0;
int vysota=1;
int timehms[3];
//int tms[250]
...
void OnRedraw(MAIN_GUI *data) // OnRedraw
{
  ...
  if (entime==1)
  {wsprintf(ws,"\r\r\r\r\r\r\r%i:%i%i/%i:%i%i",timemin,timesec/10,timesec%10,
            mp3length/60,(mp3length%60)/10,(mp3length%60)%10);
  ...
  if (playing==1) {sprintf(name,"%s%s",charofskins[curskin],"\\stop.png");DrawImg(62,19,(int)name);}
  else {sprintf(name,"%s%s",charofskins[curskin],"\\play.png");DrawImg(62,19,(int)name);}
...
int findmp3length(char *playy)
{
  ...
  char g[677];
  unsigned int err;
  f=fopen(playy, A_ReadOnly, P_READ, & err );
  fread(f,g,677,&err);
  fclose(f,&err);
  unsigned long j;
  for (int i=0;i<677;i++) {
if ((g[i]==0x49)&&(g[i+1]==0x6E)&&(g[i+2]==0x66)&&(g[i+3]==0x6F)&&(g[i+7]==0x0F))
{j=(g[i+9]*65536*256+g[i+10]*65536+g[i+11]*256+g[i+12])/9800;
return(j);}}
  return(0);
}
...
void Play(const char *fname)
{
  if (!IsCalling())
  {
    ...
    _sfo1.unk5=1;
    PLAY_ID=PlayFile(0x0, sndPath, sndFName, 0,MMI_CEPID, MSG_PLAYFILE_REPORT, &_sfo1);
#else
    _sfo1.unk5=1;
    PLAY_ID=PlayFile(0x0, sndPath, sndFName, MMI_CEPID, MSG_PLAYFILE_REPORT, &_sfo1);
#endif      
      ...
      GetDateTime(&date,&ktime);
      timehms[2]=ktime.sec;
      PlayMelody_ChangeVolume(PLAY_ID,cv);
...
{if (random==0){if (cp>kilkist-2) {cp=0;}else {cp++;}}else {cp=whattoplaynext();}
     ...
     mp3length=findmp3length(s);}
}

void keyhookvoidvolmm()
{
  ringoff=0;
if (cv!=0){cv--;PlayMelody_ChangeVolume(PLAY_ID,cv);}showvolume();
}

void keyhookmutem1()
{if (ringoff==0){PlayMelody_ChangeVolume(PLAY_ID,0);ringoff=1;REDRAW();}else{PlayMelody_ChangeVolume(PLAY_ID,cv);ringoff=0;REDRAW();};}

void keyhookvoidvolpp()
{
  ringoff=0;
if(cv!=14){cv++;PlayMelody_ChangeVolume(PLAY_ID,cv);}showvolume();
}
...
int my_keyhook(int submsg, int msg)
{
  ...
  case(LONG_PRESS):
    {switch(submsg)
  {
  case 0x14: break;//Верхня
  case 0x15: break;//Нижня   
  case 0x27: break;//Гарнітура
  };}break;
  }
  return(2);
}
...
int OnKey(MAIN_GUI *data, GUI_MSG *msg) //OnKey
{
  if (msg->gbsmsg->msg==KEY_DOWN)
  {
    ...
    {
    case RED_BUTTON:{PlayMelody_StopPlayback(PLAY_ID);
                     GBS_DelTimer(&play);
                     GBS_DelTimer(&red);
                     GBS_DelTimer(&chas);
                     GBS_DelTimer(&fv);
    ...
    case LEFT_SOFT:if (playing!=0){PlayMelody_StopPlayback(PLAY_ID);REDRAW();playing=0;break;}else
    case ENTER_BUTTON:case'5':{if (playing==1){PlayMelody_PausePlayback(PLAY_ID);playing=2;}
    else{if (playing==0){char s[256];sprintf(s,"%s%s",spysok[0][cp],spysok[1][cp]);playing=1;Play(s);}
    else{if (playing==2){PlayMelody_ResumePlayback(PLAY_ID);playing=1;}}}}REDRAW();break;
case'*':keyhookmutem1();break;
    case 0x14:case UP_BUTTON:case'2':ringoff=0;if (ringoff==1){PlayMelody_ChangeVolume(PLAY_ID,cv);}else{if (cv!=14){cv++;PlayMelody_ChangeVolume(PLAY_ID,cv);}}REDRAW();break;
    case 0x15:case DOWN_BUTTON:case'8':ringoff=0;if (ringoff==1){PlayMelody_ChangeVolume(PLAY_ID,cv);}else{if (cv!=0){cv--;PlayMelody_ChangeVolume(PLAY_ID,cv);}}REDRAW();;break;
    case LEFT_BUTTON:case'4':{if (random==0){if (cp<1) {cp=kilkist-1;}else {cp--;}}else{cp=whattoplaynext();}
     char s[256];sprintf(s,"%s%s",spysok[0][cp],spysok[1][cp]);playing=1;mp3length=findmp3length(s);Play(s);};break;
      ...
      case 5:case 7:
        {
            ...
            if (playing==1){
            if (random==0){if (cp>kilkist-2) {cp=0;}else {cp++;}}else {cp=whattoplaynext();}
            char s[256];sprintf(s,"%s%s",spysok[0][cp],spysok[1][cp]);mp3length=findmp3length(s);Play(s);}
        }
        break;
