...
int ReadPatch(char *fname)
   {
    char *buf,*str;
    FSTATS fstat;
	GetFileStats(fname, &fstat, 0);
	int f=fopen(fname,A_ReadOnly + A_BIN, P_READ, 0);

	if(f==-1)return -1;
	buf =(char*)malloc(fstat.size);
    buf[fread(f,buf,fstat.size,0)]=0;
	fclose(f,0);
    ...
    while(buf[buf_pos] && buf_pos <fstat.size)
        {
          if (buf[buf_pos]=='\n' || buf[buf_pos]=='\r')
          {
            if (lineSize > 0)
            {
              str = (char*)malloc(lineSize);
              memcpy(str, line, lineSize);
              str[lineSize] = NULL;
              lineSize = NULL;
			  if(str[0]==';'||(str[0]=='/'&&str[1]=='/'));else;/*func(str);*/mfree(str);
			}
          }
          else
