...
int LoadConfigData(const char *fname)
{
  ...
  if ((f=fopenA(fname,A_ReadOnly+A_BIN,P_READ,&ul))!=-1)
  {
  ...
  {
  L_SAVENEWCFG:
    if ((f=fopenA(fname,A_ReadWrite+A_Create+A_Truncate,P_READ+P_WRITE,&ul))!=-1)
    {
...
void InitConfig()
{
  if (LoadConfigData("4:\\ZBin\\etc\\ArcanoidMapMaker_0.1.bcfg")<0)
  {
    LoadConfigData("0:\\ZBin\\etc\\ArcanoidMapMaker_0.1.bcfg");
  }
}
