...
//#define DBG//если у вас эп 3.0(с рисованием) можете раскоментить и выводить всякую ересь
...
int pid_from=0,msg3=0,submess=0,data0=0,data1=0;
void TimeDraw(LCDLAYER *layer)
{
  ...
  //  wsprintf(ws, "is_call %d sec %d min %d mode:%d number%d", is_call,real_time.sec,real_time.min,opmod,number);
    wsprintf(ws, "pid_from:%d msg:%d submess:%d data0:%d data1:%d", pid_from,msg3,submess,data0,data1);
    DrwText(ws, layer, 0, 0, layer->w - 1, layer->h - 1, FONT_SMALL, 2, red,  black);
    FreeWS(ws);
