...
void save_image_in_cache(int n)
{
   ...
   if(tag=(char*)findtag(buf,"Content-Lat:"))
  {
    valuetag(tag, filename, sizeof(filename)-1);
    latitude=str_to_double(filename);
  }

  if(tag=(char*)findtag(buf,"Content-Lon:"))
  {
    valuetag(tag, filename, sizeof(filename)-1);
    longitude=str_to_double(filename);
  }

  if(tag=(char*)findtag(buf,"Content-Length:"))
  {
    valuetag(tag, filename, sizeof(filename)-1);
  ...
  fwrite(buf+(pbuf-size_image),size_image,1,f);
