...
//from stdlib.h respect Gudvin aka twitch
long labs(long i)
      ...
      enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      /* mfree phase dispersion adaption */
 ...
 *
 * Function:
 *    The memory used for state memory is mfreed
 *
 * Returns:
 *    Void
 */
static void Decoder_amr_exit( Decoder_amrState **state )
{
   if ( state == NULL || *state == NULL )
      return;
   mfree( ( *state )->lsfState );
   mfree( ( *state )->ec_gain_p_st );
   mfree( ( *state )->ec_gain_c_st );
   mfree( ( *state )->pred_state );
   mfree( ( *state )->background_state );
   mfree( ( *state )->ph_disp_st );
   mfree( ( *state )->Cb_gain_averState );
   mfree( ( *state )->lsp_avg_st );
   mfree( ( *state )->dtxDecoderState );
...
void Speech_Decode_Frame_exit( void **st )
{
   ...
   /* deallocate memory */
   mfree( (( Speech_Decode_FrameState * )st) );
   return;
}
